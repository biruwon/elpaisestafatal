...
async function renderDeputies() {
  ...
  main.innerHTML = '<div class="hero"><div><p class="eyebrow">Personas</p><h1>Diputados</h1><p class="hero-copy">Busca entre quienes han servido en la XV Legislatura. Las métricas describen actividad publicada y evidencia disponible, no una puntuación de trabajo.</p></div></div>' +
    ...
    '<div class="table-wrap"><table><thead><tr><th>Diputado</th><th>Grupo</th><th>Servicio</th><th>Intervenciones</th><th>Días</th><th>Minutos de intervalo publicado</th><th>Votos</th><th>Días desconocidos</th></tr></thead><tbody>' +
...
async function renderDeputy(id) {
  ...
  const d = bundle.deputy, m = bundle.metrics, daily = bundle.dailyPresence || [], officeholder = bundle.officeholderInterventions || [];
  const officeholderNote = officeholder.length ? '<div class="notice" style="margin-top:16px">' + fmt(officeholder.length) + ' comparecencias oficiales aparecen fuera del periodo de servicio como diputado. Se conservan como actividad de la persona en su cargo, pero no cuentan en las métricas de diputación.</div>' : '';
  main.innerHTML = '<a class="back" href="#/deputies">← Todos los diputados</a><div class="profile-head"><div class="profile-person">' + avatar(d, true) + '<div><p class="eyebrow">' + esc(d.group) + ' · ' + esc(d.constituency) + '</p><h1>' + esc(d.name) + '</h1><p class="profile-meta">Servicio: ' + esc((d.service || {}).from || '—') + ' — ' + esc((d.service || {}).to || 'actualidad') + ' · <a href="' + esc(d.profileUrl || d.sourceUrl) + '" target="_blank" rel="noreferrer">ficha oficial ↗</a></p></div></div><a class="button secondary" href="#/deputy/' + encodeURIComponent(id) + '/daily">Abrir detalle diario →</a></div>' +
    '<div class="grid grid-4" style="margin-top:25px">' + card('Intervenciones', fmt(m.speechTurns), 'turnos atribuidos') + card('Días de voz', fmt(m.speakingDays), 'con al menos una intervención') + card('Intervalo publicado', fmt(m.evidencedMinutes), 'minutos de intervalo publicado') + card('Votos', fmt(m.votesCast), 'sin contar “No vota”') + '</div>' + officeholderNote +
...
async function renderCoverage() {
  ...
  main.innerHTML = '<div class="hero"><div><p class="eyebrow">Transparencia</p><h1>Cobertura y fuentes</h1><p class="hero-copy">Qué se descargó, cuándo se capturó, qué hash tiene cada artefacto y qué huecos permanecen abiertos.</p></div><a class="button secondary" href="' + API + '/coverage" target="_blank">JSON completo ↗</a></div>' +
    '<div class="grid grid-3">' + card('Atribución', pct(a.interventionsWithDeputyId, a.interventionsTotal), fmt(a.interventionsWithDeputyId) + ' de ' + fmt(a.interventionsTotal) + ' intervenciones') + card('Filas deduplicadas', fmt(a.duplicateRowsCollapsed || 0), 'alias de fuente conservados') + card('Fuera de servicio', fmt(a.interventionsOutsideDeputyService || 0), 'comparecencias de cargos separadas') + card('Comisiones', '2.323', 'asignaciones importadas') + card('Vídeo revisado', '3 frames', 'muestra manual · sin identificación facial') + '</div>' +
...
const metricLabel = { speechTurns: 'Intervenciones', votesCast: 'Votos registrados', votingParticipation: 'Participación en votos', minutes: 'Minutos de intervalo publicado', initiativeCount: 'Iniciativas vinculadas', topicCount: 'Temas distintos', groupAgreement: 'Coincidencia con grupo' };
...
async function renderAnalysis() {
  ...
  main.innerHTML = '<div class="hero"><div><p class="eyebrow">Lecturas comparables</p><h1>Análisis de actividad</h1><p class="hero-copy">Rankings y relaciones calculadas sobre registros oficiales. Cada métrica conserva su denominador; no es una puntuación única de rendimiento.</p></div><a class="button secondary" href="' + API + '/analytics" target="_blank">Ver datos JSON ↗</a></div>' +
    ...
    '<div class="toolbar"><input class="input" id="analysisSearch" value="' + esc(query) + '" placeholder="Filtrar diputado, grupo o circunscripción"><select class="input" id="analysisSort"><option value="speechTurns"' + (sort === 'speechTurns' ? ' selected' : '') + '>Más intervenciones</option><option value="votesCast"' + (sort === 'votesCast' ? ' selected' : '') + '>Más votos</option><option value="votingParticipation"' + (sort === 'votingParticipation' ? ' selected' : '') + '>Mayor participación en votos</option><option value="minutes"' + (sort === 'minutes' ? ' selected' : '') + '>Más minutos de intervalo publicado</option><option value="initiativeCount"' + (sort === 'initiativeCount' ? ' selected' : '') + '>Más iniciativas</option><option value="topicCount"' + (sort === 'topicCount' ? ' selected' : '') + '>Más temas</option></select><span class="subtle" style="align-self:center">' + fmt(rows.length) + ' diputados</span></div>' +
