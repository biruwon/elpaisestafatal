...
The web interface includes searchable deputy cards, per-deputy evidence timelines, deputy-specific daily evidence (`/diputados/:id/presencia`), session pages plus joined session evidence (`/sesiones/:id/evidencia`), initiative indexes plus detail pages (`/iniciativas/:id`) with authorship and procedural history, organ indexes plus detail pages (`/organos/:id`) with dated composition and activity, committee memberships, voting results, an activity comparison table, topic evidence (`/temas`), a deputy-by-day presence explorer (`/presencia/diaria`), a timestamped video-pilot table (`/presencia/video-pilot`) and manually reviewed frame evidence (`/presencia/video-pilot/review`), provenance links, JSON endpoints and CSV exports. The joined deputy resource `/api/congreso/v1/deputies/:id` returns profile, service, substitutions, memberships, metrics and daily evidence together. CSV downloads cover deputies, metrics, sessions, agenda items, interventions, initiatives, committees, transcripts, votes, daily presence, video observations and reviewed video frames. Aggregate deputy metrics are available at `/api/congreso/v1/deputy-metrics` and `/export/deputy-metrics.csv`; machine-readable coverage is at `/api/congreso/v1/coverage` and the evidence hierarchy at `/api/congreso/v1/methodology`; `votesCast` excludes published “No vota” rows, while `nominalRows` retains the complete individual-row count. It renders the current normalized Legislature XV snapshot committed in `data/current.json` and overlays the compressed full roll-call archive at startup. Official initiative feeds are merged with intervention-derived records, preserving published authors, processing status, commission and procedural history.
