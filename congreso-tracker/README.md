...
The tracker can also run as an independent container: `docker build -t congreso-tracker . && docker run --rm -p 8787:8787 congreso-tracker`. The image contains the normalized snapshot and compressed evidence artifacts, while raw downloads and temporary media stay outside the image.

The web interface includes searchable deputy cards, per-deputy evidence timelines, deputy-specific daily evidence (`/diputados/:id/presencia`), session pages, initiative and organ indexes, committee memberships, voting results, an activity comparison table, topic evidence (`/temas`), a deputy-by-day presence explorer (`/presencia/diaria`), a timestamped video-pilot table (`/presencia/video-pilot`), provenance links, JSON endpoints and CSV exports. CSV downloads cover deputies, metrics, sessions, agenda items, interventions, initiatives, committees, transcripts, votes, daily presence and video observations. Aggregate deputy metrics are available at `/api/congreso/v1/deputy-metrics` and `/export/deputy-metrics.csv`; machine-readable coverage is at `/api/congreso/v1/coverage` and the evidence hierarchy at `/api/congreso/v1/methodology`; `votesCast` excludes published “No vota” rows, while `nominalRows` retains the complete individual-row count. It renders the current normalized Legislature XV snapshot committed in `data/current.json` and overlays the compressed full roll-call archive at startup. Official initiative feeds are merged with intervention-derived records, preserving published authors, processing status, commission and procedural history.
