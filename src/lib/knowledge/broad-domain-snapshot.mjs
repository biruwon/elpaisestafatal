...
const packets = [
  ...
  },
  {
    ...
    criteria: [
      ...
      { id: 'pension-expenditure-total', familyId: 'pensions', familyLabel: 'Pensiones', label: 'Gasto total de vejez y supervivencia', finding: 'El gasto total localizado cubre prestaciones de vejez y supervivencia de todos los esquemas. No es un desglose del gasto contributivo neto observado; la proyección contributiva de AIReF y el presupuesto C9.4 se muestran por separado.', metricIds: ['old_age_survivors_benefits_total'], population: 'gasto en prestaciones de protección social para vejez y supervivencia', denominator: 'ninguno; importe agregado', unit: 'millones de euros', fallbackData: ['205.009,77 millones de euros en prestaciones de vejez y supervivencia (2024)'], missingDimensions: ['gasto contributivo neto observado'], sourceIds: ['pension-expenditure-total-source'] },
      ...
      { id: 'pension-system-balance', familyId: 'pensions', familyLabel: 'Pensiones', label: 'Saldo observado del sistema', finding: 'La serie anual completa disponible en esta comprobación es una proyección de AIReF. Sigue faltando un balance anual observado que impute todos los ingresos, gastos y transferencias exclusivamente al sistema de pensiones.', population: 'sistema de pensiones y sus ingresos, gastos y transferencias observados', denominator: 'importe agregado del sistema', missingDimensions: ['saldo anual observado del sistema de pensiones'], sourceIds: ['pension-spending-source', 'social-protection-receipts-source', 'social-security-general-account-2024'] },
      { id: 'social-security-account', familyId: 'pensions', familyLabel: 'Pensiones', label: 'Cuenta presupuestaria de la Seguridad Social', dataKind: 'observed', finding: 'La Cuenta General 2024 permite observar el gasto reconocido del programa de pensiones contributivas y el resultado de la cuenta consolidada de la Seguridad Social. Estas cifras tienen alcances distintos y no forman por sí solas un saldo pensionista completo.', metricIds: ['social_security_contributory_pension_expenditure', 'social_security_current_revenue', 'social_security_current_expenditure', 'social_security_current_balance', 'social_security_contributions', 'social_security_current_transfers', 'social_security_budget_total_balance'], population: 'programa de pensiones contributivas y entidades gestoras y servicios comunes de la Seguridad Social', denominator: 'importe agregado', unit: 'millones de euros', fallbackData: ['Gasto reconocido en pensiones contributivas: 172.653,50 millones de euros (2024)', 'Ingresos corrientes de entidades gestoras y servicios comunes: 195.891,23 millones de euros (2024)', 'Gastos corrientes de entidades gestoras y servicios comunes: 205.201,38 millones de euros (2024)', 'Resultado corriente de la Seguridad Social: -9.310,15 millones de euros (2024)', 'Cotizaciones sociales: 146.148,70 millones de euros (2024)', 'Transferencias corrientes de la Seguridad Social: 48.151,75 millones de euros (2024)', 'Resultado presupuestario total de la Seguridad Social: -2.900,90 millones de euros (2024)'], missingDimensions: ['imputación observada de ingresos exclusivamente a pensiones', 'saldo observado completo del sistema'], sourceIds: ['social-security-general-account-2024'] },
      { id: 'pension-state-transfers-budget', familyId: 'pensions', familyLabel: 'Pensiones', label: 'Transferencias presupuestadas a pensiones', dataKind: 'context', finding: 'El cuadro C3 de la Seguridad Social identifica partidas presupuestadas por destino. Permite mostrar transferencias previstas para complementos a mínimos, pensiones no contributivas y el Pacto de Toledo, pero no equivale a transferencias ejecutadas ni a un balance anual del sistema.', metricIds: ['social_security_minimum_complements_transfer_budget', 'social_security_noncontributory_transfer_budget', 'social_security_pact_toledo_transfer_budget'], population: 'transferencias del Estado presupuestadas por destino dentro de la Seguridad Social', denominator: 'ninguno; importe presupuestado', unit: 'millones de euros', fallbackData: ['Complementos a mínimos: 7.261,17 millones de euros presupuestados (2025P)', 'Pensiones no contributivas: 3.002,96 millones de euros presupuestados (2025P)', 'Pacto de Toledo: 19.888,00 millones de euros presupuestados (2025P)'], missingDimensions: ['transferencias ejecutadas y atribuidas exclusivamente al sistema de pensiones'], sourceIds: ['social-security-pension-transfers-2025p'] },
      { id: 'pension-finance-projection-series', familyId: 'pensions', familyLabel: 'Pensiones', label: 'Serie anual de financiación y saldo', dataKind: 'projected', finding: 'AIReF publica una serie anual 2020–2070 de ingresos, gastos, saldo y transferencias del sistema modelizado. Es evidencia cuantitativa de un escenario, no un saldo observado ni una predicción segura.', metricIds: ['pension_contributory_income_projected', 'pension_public_transfers_projected', 'pension_contributory_expenditure_projected', 'pension_noncontributory_expenditure_projected', 'pension_system_income_projected', 'pension_system_expenditure_projected', 'pension_system_balance_projected', 'pension_implicit_transfers_projected'], population: 'sistema público de pensiones modelizado por AIReF', denominator: 'PIB', unit: '% del PIB', fallbackData: ['Cotizaciones dedicadas a pensiones: 9,20 % del PIB (2020) → 10,37 % (2070)', 'Transferencias de la Administración Central: 0,87 % del PIB (2020) → 1,70 % (2070)', 'Gasto de pensiones SS y complementos a mínimos: 12,33 % del PIB (2020) → 14,86 % (2070)', 'Pensiones no contributivas: 0,23 % del PIB (2020) → 0,24 % (2070)', 'Ingresos totales: 11,54 % del PIB (2020) → 12,40 % (2070)', 'Gastos totales: 14,02 % del PIB (2020) → 15,42 % (2070)', 'Saldo anual: -2,48 % del PIB (2020) → -3,02 % (2070)', 'Transferencias implícitas: 2,48 % del PIB (2020) → 3,02 % (2070)'], sourceIds: ['airef-pension-projection-tables-2026'] },
...
const answerPlanForPacket = (packet, { now = Date.now(), observations = [] } = {}) => {
  ...
  const conversationReply = packet.replyProfile === 'pension-sustainability'
    ? [
      `${packet.headline}.`,
      observedFindings.length ? `En 2024, la Cuenta General registra ${observedFindings.slice(0, 3).map(replyFinding).join('; ')}. Son medidas de la Seguridad Social y del programa contributivo con alcances distintos; no forman por sí solas un saldo pensionista completo.` : undefined,
      transferBudgetFindings.length ? `El presupuesto 2025P prevé ${transferBudgetFindings.map(replyFinding).join('; ')}; son importes presupuestados por destino, no transferencias ejecutadas.` : undefined,
      projectedFindings.length ? `En el escenario de AIReF para 2020–2070, ${projectedFindings.slice(0, 2).map(replyFinding).join('; ')}.` : undefined,
      'Estos datos muestran presión y necesidades de financiación, pero no demuestran por sí solos una insostenibilidad total ni que las pensiones ya hayan arruinado las cuentas públicas. La evidencia observada no publica una cuenta anual que impute todos los ingresos, gastos y transferencias exclusivamente a pensiones; esa es la principal limitación de atribución.',
    ].filter(Boolean).join(' ')
    : [
      `${packet.headline}.`,
