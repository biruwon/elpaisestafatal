...
const packets = [
  ...
  },
  {
    ...
    criteria: [
      ...
      { id: 'pension-balance', familyId: 'pensions', familyLabel: 'Pensiones', label: 'Gasto por habitante', finding: 'La sostenibilidad requiere comparar ingresos contributivos, gasto, transferencias y compromisos futuros; el gasto actual aislado no decide el resultado.', metricIds: ['old_age_survivors_benefits_per_capita'], population: 'gasto en prestaciones de protección social para vejez y supervivencia', denominator: 'población residente', unit: '€ por habitante en prestaciones de vejez y supervivencia', fallbackData: ['4.194,66 € por habitante en prestaciones de vejez y supervivencia (2024)', '3.293,16 € por habitante en prestaciones de vejez y supervivencia (2020)'], missingDimensions: ['saldo del sistema', 'proyección de ingresos y gastos'], sourceIds: ['pension-spending-source'] },
...
const answerPlanForPacket = (packet, { now = Date.now(), observations = [] } = {}) => {
  ...
  const quantitativeEntries = matchedObservations.flatMap(({ criterion, observations: items }, index) => {
    ...
    return [
      ...entries.map((text) => ({ text, familyId: criterion.familyId || criterion.id, dataKind: criterion.dataKind || (items.length ? 'observed' : 'snapshot') })),
      ...(!fallbackIsNewer(criterion, index) ? additional.map((value) => ({ text: `${criterion.label}: ${cleanQuantitative(value)}`, familyId: criterion.familyId || criterion.id, dataKind: criterion.dataKind || 'snapshot' })) : []),
    ];
  });
