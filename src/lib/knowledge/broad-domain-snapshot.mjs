...
const packets = [
  ...
  },
  {
    ...
    criteria: [
      ...
      { id: 'pension-balance', familyId: 'pensions', familyLabel: 'Pensiones', label: 'Gasto por habitante', finding: 'La sostenibilidad requiere comparar ingresos contributivos, gasto, transferencias y compromisos futuros; el gasto actual aislado no decide el resultado.', metricIds: ['old_age_survivors_benefits_per_capita'], population: 'gasto en prestaciones de protección social para vejez y supervivencia', denominator: 'población residente', unit: '€ por habitante en prestaciones de vejez y supervivencia', fallbackData: ['3.293,16 € por habitante en prestaciones de vejez y supervivencia (2020)'], missingDimensions: ['proyección de ingresos y gastos'], sourceIds: ['pension-spending-source'] },
...
const answerPlanForPacket = (packet, { now = Date.now(), observations = [] } = {}) => {
  ...
  const snapshotFindings = compactFindings(quantitativeEntries.filter((entry) => entry.dataKind === 'snapshot' || entry.dataKind === 'context'));
  const findingSections = [
    ...
    snapshotFindings.length ? `Datos de referencia revisados: ${snapshotFindings.slice(0, 4).join('; ')}.` : undefined,
  ].filter(Boolean);
