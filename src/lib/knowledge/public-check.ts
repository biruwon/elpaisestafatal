...
export type CheckVisual = { type: 'line' | 'bar' | 'comparison' | 'money-flow'; title?: string; unit?: string; labels: string[]; values: number[]; evidenceIds: string[]; sourceId?: string; note?: string; interpretation?: string; breakAfter?: number[] };
export type EvidenceStatus = 'available' | 'partial' | 'missing';
export type EvidenceDimensions = { subject?: string; population?: string; period?: string; geography?: string; denominator?: string; unit?: string; causalRequirement?: string };
export type PublicEvidenceCriterion = { id: string; criterionId?: string; label: string; finding: string; status?: EvidenceStatus; dataKind?: 'observed' | 'projected' | 'snapshot' | 'context'; dimensions?: EvidenceDimensions; evidenceIds?: string[]; sourceIds?: string[]; data?: string[]; missingDimensions?: string[] };
export type PublicEvidenceFamily = { familyId: string; familyLabel: string; researchNote?: string; label: string; direction: 'supports' | 'qualifies' | 'contradicts' | 'neutral'; status?: EvidenceStatus; dataKind?: 'observed' | 'projected' | 'snapshot' | 'context'; dimensions?: EvidenceDimensions; evidenceIds: string[]; sourceIds: string[]; finding?: string; limitation?: string; period?: string; data?: string[]; missingDimensions?: string[]; criteria?: PublicEvidenceCriterion[] };
...
export type CheckResult = { claim: string; interpretation?: ClaimInterpretation; thesis?: { text: string; kind: 'factual' | 'evaluative'; conclusion: string; criteria?: string[] }; reply: string; answer: string; shareableReply?: string; shareableSourceIds?: string[]; evidenceLevel?: EvidenceLevel; keyFact?: string; criteria?: CheckCriterion[]; arguments?: ArgumentAssessment[]; coverageSummary?: { total: number; supported: number; contradicted: number; mixed: number; insufficient: number; notVerifiable: number }; whatWeKnow: string[]; limitations: string[]; scope: CheckScope; sources: CheckSource[]; assessment?: ClaimAssessment; visual?: CheckVisual; visuals?: CheckVisual[]; scorecard?: CheckScorecard; canonicalHref?: string; canonicalSlug?: string; evidenceSummary?: PublicEvidenceSummary };
