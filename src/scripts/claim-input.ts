...
const researchPlanMarkup = (plan: AnswerPlan): string => {
  const research = plan.researchPlan;
  if (!research) return '';
  const propositions = research.propositions?.slice(0, 6) || [];
  const needs = propositions.flatMap((item) => item.evidenceNeeded || []).slice(0, 8);
  return `<section class="claim-plan-research claim-plan-card" data-result-target="research"><span class="clarification-label">Cómo se puede comprobar</span><p>Esta es la ruta de investigación para tu frase; todavía no es un veredicto.</p>${propositions.length ? `<div><strong>Proposiciones separadas</strong><ul>${propositions.map((item) => `<li><span>${escapeHtml(item.id)}</span>${item.evidenceNeeded?.length ? `<small>${escapeHtml(item.evidenceNeeded.join(' · '))}</small>` : ''}</li>`).join('')}</ul></div>` : ''}${needs.length ? `<div><strong>Qué evidencia falta</strong><ul>${needs.map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul></div>` : ''}${research.requiredDimensions?.length ? `<div><strong>Dimensiones necesarias</strong><p>${escapeHtml(research.requiredDimensions.join(' · '))}</p></div>` : ''}${research.neutralQueries?.length ? `<details><summary>Consultas neutrales previstas</summary><ul>${research.neutralQueries.map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul></details>` : ''}</section>`;
};
...
const renderScorecardPlan = (original: string, plan: AnswerPlan): void => {
  ...
  const formatValue = (item: (typeof scorecard.items)[number], value?: string): string => {
    if (value == null) return '—';
    const numeric = Number(value);
    if (!Number.isFinite(numeric)) return value;
    const unit = String((item as typeof item & { unit?: string }).unit || '');
    const formatted = new Intl.NumberFormat('es-ES', { maximumFractionDigits: 1 }).format(numeric);
    return unit.includes('%') ? `${formatted} %` : unit.includes('euros') ? `${formatted} €` : formatted;
  };
  const rows = scorecard.items.map((item) => `<details class="scorecard-row" data-direction="${item.direction}"><summary><span class="scorecard-row-label"><strong>${escapeHtml(item.label)}</strong><b class="scorecard-badge scorecard-${item.direction}">${labels[item.direction]}</b></span><span class="scorecard-row-values">${item.baseline ? `${escapeHtml(formatValue(item, item.baseline.value))} → ` : ''}${item.comparison ? escapeHtml(formatValue(item, item.comparison.value)) : '—'}${item.change ? ` <small>${escapeHtml(item.change)}</small>` : ''}</span></summary><div class="scorecard-row-detail"><p>${item.baseline ? `Base ${escapeHtml(item.baseline.period)}: ${escapeHtml(formatValue(item, item.baseline.value))} · ` : ''}${item.comparison ? `Último dato ${escapeHtml(item.comparison.period)}: ${escapeHtml(formatValue(item, item.comparison.value))}.` : 'No hay dos observaciones compatibles.'}</p>${item.caveat ? `<p>${escapeHtml(item.caveat)}</p>` : ''}${item.evidenceIds?.length ? `<div class="scorecard-row-sources">${item.evidenceIds.map((id) => sources.get(id)).filter((source): source is NonNullable<typeof source> => Boolean(source)).map((source) => `<a href="${escapeHtml(source.url)}" target="_blank" rel="noreferrer">${escapeHtml(source.title)} ↗</a>`).join('')}</div>` : ''}</div></details>`).join('');
  result.innerHTML = `<article class="scorecard-result" aria-labelledby="scorecard-title"><div class="scorecard-submitted"><span>Comprobando</span><strong>${escapeHtml(original)}</strong><button type="button" data-scorecard-edit>Editar</button></div><p class="eyebrow">Cuadro de indicadores</p><h3 id="scorecard-title">Los datos no apoyan que “todo vaya peor”</h3><p class="scorecard-answer">En este cuadro, <strong>${improved} de ${scorecard.items.length}</strong> indicadores mejoran y <strong>${worsened}</strong> empeora entre ${escapeHtml(scorecard.baseline.period)} y ${escapeHtml(scorecard.comparison.period)}.</p><p class="scorecard-caveat">Esto describe cambios observados; no demuestra qué políticas los causaron ni produce una nota partidista.</p><div class="scorecard-meta">España · ${escapeHtml(scorecard.baseline.period)} → ${escapeHtml(scorecard.comparison.period)} · Datos revisados${plan.asOf ? ` · ${escapeHtml(new Date(plan.asOf).toLocaleDateString('es-ES'))}` : ''}</div><section class="scorecard-list" aria-label="Indicadores comparados">${rows}</section><details class="scorecard-sources"><summary>Fuentes y método <small>${plan.sourceLinks?.length || 0} fuentes · ${plan.asOf ? escapeHtml(new Date(plan.asOf).toLocaleDateString('es-ES')) : ''}</small></summary><p>${escapeHtml((scorecard as typeof scorecard & { assumption?: string }).assumption || plan.limitation || 'Se compara el último año completo anterior con el último dato compatible.')}</p>${(plan.sourceLinks || []).map((source) => `<a href="${escapeHtml(source.url)}" target="_blank" rel="noreferrer">${escapeHtml(source.title)} · ${escapeHtml(source.publisher || '')} ↗</a>`).join('')}</details><div class="scorecard-actions"><button type="button" data-scorecard-edit>Editar afirmación</button><button type="button" data-new-check>Nueva comprobación</button></div></article>`;
  const scorecardTitle = result.querySelector<HTMLElement>('#scorecard-title');
  if (scorecardTitle && !/gobierno|izquierda|derecha|gobernando/i.test(original)) scorecardTitle.textContent = 'Este tema necesita varios indicadores, no un veredicto único';
  const topicEvidence = (plan.blocks as Array<{ type?: string; topics?: Array<{ label?: string; href?: string; observations?: Array<{ metric?: string; value?: number | string; unit?: string; period?: string }> }> }>).find((block) => block.type === 'condition_topic_evidence');
  const scopeNotice = (plan.blocks as Array<{ type?: string; text?: string }>).find((block) => block.type === 'scope_notice');
  if (scopeNotice?.text) result.querySelector('.scorecard-answer')?.insertAdjacentHTML('afterend', `<p class="scorecard-caveat">${escapeHtml(scopeNotice.text)}</p>`);
  const populationContext = (plan.blocks as Array<{ type?: string; population?: string; observations?: Array<{ metric?: string; value?: number | string; unit?: string; period?: string }> }>).find((block) => block.type === 'population_context');
  if (populationContext?.observations?.length) {
    const html = populationContext.observations.slice(0, 6).map((observation) => `<article class="scorecard-topic"><strong>${escapeHtml(observation.metric || '')}</strong><p>${escapeHtml(String(observation.value ?? ''))}${observation.unit ? ` ${escapeHtml(observation.unit)}` : ''} · ${escapeHtml(observation.period || '')}</p></article>`).join('');
    result.querySelector('.scorecard-sources')?.insertAdjacentHTML('beforebegin', `<section class="scorecard-topics" aria-label="Contexto de población"><p class="scorecard-meta">Contexto disponible para ${escapeHtml(populationContext.population || 'esta población')}</p>${html}</section>`);
  }
  if (topicEvidence?.topics?.length) {
    const html = topicEvidence.topics.filter((topic) => topic.observations?.length).map((topic) => `<article class="scorecard-topic"><a href="${escapeHtml(topic.href || '#')}"><strong>${escapeHtml(topic.label || '')}</strong> ↗</a><p>${topic.observations?.slice(0, 3).map((observation) => `${escapeHtml(observation.metric || '')}: ${escapeHtml(String(observation.value ?? ''))}${observation.unit ? ` ${escapeHtml(observation.unit)}` : ''} (${escapeHtml(observation.period || '')})`).join(' · ')}</p></article>`).join('');
    if (html) result.querySelector('.scorecard-sources')?.insertAdjacentHTML('beforebegin', `<section class="scorecard-topics" aria-label="Temas relacionados"><p class="scorecard-meta">Temas que puedes abrir</p>${html}</section>`);
  }
