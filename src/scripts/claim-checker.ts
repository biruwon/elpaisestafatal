...
type CheckResult = {
  ...
  assessment?: string; canonicalHref?: string; visual?: { title?: string; unit?: string; labels: string[]; values: number[] }; evidenceSummary?: { mode: 'dynamic' | 'snapshot' | 'mixed' | 'none'; families: Array<{ label: string; direction: string; evidenceIds: string[] }>; missingDimensions?: string[]; fallbackReason?: string };
};
type CheckResponse =
...
const renderResult = (response: Extract<CheckResponse, { state: 'supported' | 'limited' | 'insufficient' }>): void => {
  ...
  const evidenceMode = item.evidenceSummary?.mode === 'snapshot' ? 'Respuesta basada en snapshot' : item.evidenceSummary?.mode === 'mixed' ? 'Datos dinámicos y snapshot separados' : item.evidenceSummary?.mode === 'none' ? (response.state === 'limited' ? 'Datos relacionados, pero insuficientes para la conclusión' : 'No hay datos compatibles') : item.evidenceSummary?.families?.length ? 'Comprobación directa' : '';
  const familySummary = item.evidenceSummary?.families?.length ? `<section class="claim-facts"><h3>${escapeHtml(evidenceMode || 'Familias de evidencia')}</h3>${item.evidenceSummary.families.slice(0, 5).map((family) => `<p><strong>${escapeHtml(family.label)}:</strong> ${escapeHtml(family.direction === 'qualifies' ? 'aporta contexto y matices' : family.direction === 'supports' ? 'respalda esta parte' : family.direction === 'contradicts' ? 'contradice esta parte' : 'describe esta dimensión')}</p>`).join('')}${item.evidenceSummary.fallbackReason ? `<p>${escapeHtml(item.evidenceSummary.fallbackReason)}</p>` : ''}</section>` : evidenceMode ? `<section class="claim-facts"><h3>${escapeHtml(evidenceMode)}</h3></section>` : '';
  ...
  const interpretation = item.interpretation ? `<section class="claim-interpretation"><span class="eyebrow">Entendemos la frase así</span><p>${escapeHtml(item.interpretation.normalizedClaim)}</p></section>` : '';
  const thesis = item.thesis ? `<section class="claim-facts"><div><h3>Tipo de conclusión</h3><p>${escapeHtml(item.thesis.conclusion)}</p></div><div><h3>Criterios necesarios</h3><p>${escapeHtml((item.thesis.criteria || []).join(' · '))}</p></div></section>` : '';
  const criteria = item.criteria?.length ? `<section class="claim-facts"><h3>Por qué</h3>${item.criteria.map((criterion) => `<p><strong>${escapeHtml(criterion.label)}:</strong> ${escapeHtml(criterion.finding)}</p>`).join('')}</section>` : '';
  const argumentsView = item.arguments?.length ? `<section class="claim-arguments"><h3>${item.coverageSummary ? `Argumentos comprobados (${item.coverageSummary.total})` : 'Argumentos comprobados'}</h3>${item.arguments.map((argument) => { const argumentSources = item.sources.filter((source) => argument.sourceIds?.includes(source.id)); return `<details><summary><span class="argument-verdict argument-${escapeHtml(argument.verdict)}">${escapeHtml(argument.verdict)}</span>${escapeHtml(argument.claim)}</summary><p>${escapeHtml(argument.finding)}</p><small>${argument.evidenceIds?.length ? `${argument.evidenceIds.length} registro(s) de evidencia asociado(s)` : 'Sin registro de evidencia específico'}</small>${argumentSources.length ? `<div class="argument-sources">${argumentSources.map((source) => `<a href="${escapeHtml(source.url)}" target="_blank" rel="noreferrer">${escapeHtml(source.title)} ↗</a>`).join('')}</div>` : ''}${argument.limitations?.length ? `<small>${escapeHtml(argument.limitations[0])}</small>` : ''}</details>`; }).join('')}</section>` : '';
  result.innerHTML = `<article class="claim-result" data-state="${response.state}"><div class="claim-result-heading"><div><span class="eyebrow">${stateLabel}</span><p class="claim-original">${escapeHtml(item.claim)}</p></div>${assessment}</div>${interpretation}${thesis}${scope ? `<p class="claim-scope">${escapeHtml(scope)}</p>` : ''}<section class="claim-reply"><div><p class="eyebrow">Respuesta para enviar</p><p class="claim-reply-text">${escapeHtml(item.reply)}</p></div><button type="button" class="claim-copy" data-copy-answer>Copiar respuesta</button><span class="claim-live" aria-live="polite"></span></section>${item.keyFact ? `<section class="claim-key-fact"><span class="eyebrow">Dato decisivo</span><p>${escapeHtml(item.keyFact)}</p></section>` : ''}${familySummary}${argumentsView}${criteria}${know}${visual}${sources}<div class="claim-result-footer">${item.canonicalHref ? `<a href="${escapeHtml(item.canonicalHref)}">Abrir explicación completa →</a>` : ''}<button type="button" data-new-check>Comprobar otra frase</button></div></article>`;
