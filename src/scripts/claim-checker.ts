...
type EvidenceCriterion = { id: string; label: string; finding?: string; evidenceIds?: string[]; sourceIds?: string[]; data?: string[]; missingDimensions?: string[] };
type EvidenceFamily = { familyId?: string; familyLabel?: string; label: string; direction: string; evidenceIds: string[]; sourceIds?: string[]; finding?: string; limitation?: string; period?: string; data?: string[]; missingDimensions?: string[]; criteria?: EvidenceCriterion[] };
type CheckResponse =
...
const evidenceGroupsFor = (families: EvidenceFamily[]): EvidenceFamily[] => {
  ...
  families.forEach((family, index) => {
    ...
    const criterion: EvidenceCriterion = { id: family.label, label: family.label, finding: family.finding, evidenceIds: family.evidenceIds, sourceIds: family.sourceIds, data: family.data, missingDimensions: family.missingDimensions };
...
const renderEvidenceGroups = (groups: EvidenceFamily[], sources: CheckResult['sources']): string => groups.map((group) => {
  ...
  return `<section class="evidence-family" data-family-id="${escapeHtml(group.familyId || group.label)}"><header><span class="eyebrow">Familia de evidencia</span><h4>${escapeHtml(publicMetricLabel(group.familyLabel || group.label))}</h4><p>${escapeHtml(group.finding || 'Esta familia se evalúa con sus propias medidas.')}</p></header><div class="result-evidence-grid">${criteria.map((criterion) => `<article class="evidence-criterion"><strong>${escapeHtml(publicMetricLabel(criterion.label))}</strong><p class="evidence-finding">${escapeHtml(criterion.finding || 'No hay una medición compatible localizada.')}</p>${criterion.data?.length ? `<p class="evidence-data"><span>Valores observados</span>${criterion.data.map((value) => escapeHtml(value)).join(' · ')}</p>` : ''}${criterion.missingDimensions?.length ? `<p class="evidence-gap-inline">Datos pendientes: ${escapeHtml(criterion.missingDimensions.join('; '))}</p>` : ''}</article>`).join('')}</div>${group.data?.length ? `<p class="evidence-family-data"><strong>Valores de la familia:</strong> ${group.data.map((value) => escapeHtml(value)).join(' · ')}</p>` : ''}${group.missingDimensions?.length ? `<p class="result-note evidence-missing-note"><strong>Qué queda pendiente en esta familia:</strong> ${escapeHtml(group.missingDimensions.join('; '))}.</p>` : ''}${sourceLinks.length ? `<div class="evidence-family-sources"><span class="eyebrow">Fuentes de esta familia</span>${sourceLinks.map((source) => `<a href="${escapeHtml(source.url)}" target="_blank" rel="noreferrer">${escapeHtml(source.title)} ↗</a>`).join('')}</div>` : ''}</section>`;
}).join('');
...
const renderResult = (response: Extract<CheckResponse, { state: 'supported' | 'limited' | 'insufficient' }>): void => {
  ...
  const groupedFamilies = evidenceGroups.length ? `<section class="result-section result-evidence" data-evidence-groups><div class="result-section-heading"><span class="eyebrow">Qué respaldan los datos</span><h3>${escapeHtml(item.evidenceSummary?.mode === 'none' ? 'Datos relacionados, no una prueba directa' : 'La evidencia disponible')}</h3></div>${renderEvidenceGroups(evidenceGroups, item.sources)}${item.evidenceSummary?.fallbackReason ? `<p class="result-note">${escapeHtml(item.evidenceSummary.fallbackReason)}</p>` : ''}</section>` : '';
  const compoundKnown = evidenceGroups.length ? `<section class="result-section result-two-column"><div><span class="eyebrow">Qué sabemos</span><p>${escapeHtml(evidenceGroups.map((group) => `${group.familyLabel || group.label}: ${group.data?.length ? group.data.join(', ') : group.finding || 'sin medición compatible localizada'}`).join(' · '))}</p></div><div class="result-limit"><span class="eyebrow">Qué no demuestra</span><p>${escapeHtml(item.limitations[0] || 'La evidencia de una familia no demuestra por sí sola las demás.')}</p></div></section>` : '';
  const groupedMethodology = evidenceGroups.length ? `<details class="result-details result-methodology"><summary>Cómo se ha comprobado<span aria-hidden="true">＋</span></summary><div><p>La frase se separa en familias independientes. Cada una necesita población, periodo, denominador y unidad compatibles.</p>${evidenceGroups.map((group) => `<section><h4>${escapeHtml(group.familyLabel || group.label)}</h4>${(group.criteria || []).map((criterion) => `<p><strong>${escapeHtml(criterion.label)}:</strong> ${escapeHtml(criterion.finding || 'Sin medición compatible localizada.')}</p>`).join('')}</section>`).join('')}</div></details>` : '';
