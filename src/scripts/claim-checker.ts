...
type CheckResult = {
  ...
  arguments?: Array<{ id: string; claim: string; verdict: string; finding: string; limitations: string[] }>;
...
const renderResult = (response: Extract<CheckResponse, { state: 'supported' | 'limited' | 'insufficient' }>): void => {
  ...
  const argumentsView = item.arguments?.length ? `<section class="claim-arguments"><h3>${item.coverageSummary ? `Argumentos comprobados (${item.coverageSummary.total})` : 'Argumentos comprobados'}</h3>${item.arguments.map((argument) => `<details><summary><span class="argument-verdict argument-${escapeHtml(argument.verdict)}">${escapeHtml(argument.verdict)}</span>${escapeHtml(argument.claim)}</summary><p>${escapeHtml(argument.finding)}</p>${argument.limitations?.length ? `<small>${escapeHtml(argument.limitations[0])}</small>` : ''}</details>`).join('')}</section>` : '';
