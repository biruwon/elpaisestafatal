...
const renderResult = (response: Extract<CheckResponse, { state: 'supported' | 'limited' | 'insufficient' }>): void => {
  ...
  const families = !item.scorecard && item.evidenceSummary?.families?.length ? `<section class="result-section result-evidence"><div class="result-section-heading"><span class="eyebrow">Qué respaldan los datos</span><h3>${escapeHtml(item.evidenceSummary.mode === 'none' ? 'Datos relacionados, no una prueba directa' : 'La evidencia disponible')}</h3></div><div class="result-evidence-grid">${item.evidenceSummary.families.slice(0, 5).map((family) => `<article><strong>${escapeHtml(publicMetricLabel(family.label))}</strong>${family.finding ? `<p class="evidence-finding">${escapeHtml(family.finding)}</p>` : '<p class="evidence-finding evidence-finding-missing">No hay un valor o hallazgo concreto disponible para esta dimensión.</p>'}<span class="evidence-direction evidence-direction-${escapeHtml(family.direction)}">${escapeHtml(directionLabel(family.direction))}</span>${family.period ? `<small class="evidence-period">Periodo: ${escapeHtml(family.period)}</small>` : ''}${family.limitation ? `<small class="evidence-limitation">${escapeHtml(family.limitation)}</small>` : ''}</article>`).join('')}</div>${item.evidenceSummary.fallbackReason ? `<p class="result-note">${escapeHtml(item.evidenceSummary.fallbackReason)}</p>` : ''}</section>` : '';
