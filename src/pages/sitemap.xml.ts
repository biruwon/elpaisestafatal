...
const urls = ['/', '/buscar', '/afirmaciones', '/acerca-de', '/metodologia', '/fuentes', '/revisiones', '/privacidad', ...claims.map((item) => `/afirmaciones/${item.slug}`), ...concerns.map((item) => `/preocupaciones/${item.slug}`)];
