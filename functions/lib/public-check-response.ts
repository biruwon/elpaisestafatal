...
type CriteriaProfile = { kind: 'institutional_label' | 'factual_allegation' | 'evaluative_judgment'; predicates: RegExp; criteria: string[]; defaultLimitation: string };
export const criteriaProfiles: Record<string, CriteriaProfile> = {
  'democratic-power': { kind: 'institutional_label', predicates: /dictador|dictadura|autoritari|fascista|comunista/i, criteria: ['Acceso al cargo', 'Controles institucionales', 'Competencia política'], defaultLimitation: 'Esta conclusión se refiere a la etiqueta institucional; no resuelve acusaciones sobre decisiones concretas.' },
  allegation: { kind: 'factual_allegation', predicates: /corrupt|traidor|ladr[oó]n|criminal|delincuente/i, criteria: ['Conducta concreta', 'Estado del procedimiento', 'Fuente directamente relacionada'], defaultLimitation: 'Una acusación general no permite distinguir entre opinión, investigación, acusación formal o condena.' },
  'performance-judgment': { kind: 'evaluative_judgment', predicates: /incompetent|desastre|fracaso|mentiros/i, criteria: ['Criterio evaluable', 'Indicadores relevantes', 'Periodo comparable'], defaultLimitation: 'Una valoración depende del criterio y periodo elegidos; no equivale por sí sola a un hecho.' },
};
...
const interpretationFor = (claim: string): ClaimInterpretation => { const normalizedClaim = claim.trim(); const lower = normalizedClaim.toLocaleLowerCase('es'); const institutional = lower.match(/\b(dictador|dictadura|autoritario|autoritaria|fascista|comunista)\b/); const allegation = lower.match(/\b(corrupto|corrupta|traidor|traidora|ladr[oó]n|criminal|delincuente)\b/); const evaluative = lower.match(/\b(incompetente|desastre|fracaso|mentiroso|mentirosa)\b/); const kind = institutional ? 'institutional_label' : allegation ? 'factual_allegation' : evaluative ? 'evaluative_judgment' : 'specific_fact'; return { kind, predicate: institutional?.[1] || allegation?.[1] || evaluative?.[1], normalizedClaim, criteriaProfile: institutional ? 'democratic-power' : allegation ? 'allegation' : evaluative ? 'performance-judgment' : undefined }; };
...
export const directClaimCheck = (claim: string): PublicCheckResponse | undefined => { const interpretation = directInterpretation(claim); if (!interpretation) return undefined; const lower = claim.toLocaleLowerCase('es'); if (interpretation.kind === 'institutional_label' && /dictador|dictadura/.test(lower)) { const sources: CheckSource[] = [{ id: 'constitution-spain', title: 'Constitución Española', publisher: 'BOE', url: 'https://www.boe.es/legislacion/documentos/ConstitucionCASTELLANO.pdf' }, { id: 'congress-investiture', title: 'Investidura del presidente del Gobierno', publisher: 'Congreso de los Diputados', url: 'https://www.congreso.es/cem/constitu' }]; const profile = criteriaProfiles['democratic-power']; const criteria: CheckCriterion[] = [{ id: 'office', label: profile.criteria[0], finding: 'El presidente del Gobierno necesita la investidura del Congreso.', sourceIds: ['constitution-spain', 'congress-investiture'] }, { id: 'checks', label: profile.criteria[1], finding: 'El Gobierno está sometido al Parlamento, a las leyes y a los tribunales.', sourceIds: ['constitution-spain'] }, { id: 'opposition', label: profile.criteria[2], finding: 'Existen elecciones, oposición parlamentaria y mecanismos para sustituir al Gobierno.', sourceIds: ['constitution-spain'] }]; const subject = claim.replace(/\b(es|son|es una|es un|sería|seria)\b/gi, ' ').replace(/\b(dictador|dictadura)\b/i, '').trim() || 'La persona o país mencionado'; const reply = `${subject} no encaja en la definición de dictadura: existe competencia política, controles institucionales y mecanismos legales para sustituir al Gobierno. Eso no impide examinar decisiones concretas por abuso de poder o deterioro institucional.`; return { state: 'supported', id: `interpret-${Date.now().toString(36)}`, result: { claim, interpretation, reply, answer: reply, evidenceLevel: 'supported', keyFact: criteria[0].finding, criteria, whatWeKnow: criteria.map((item) => item.finding), limitations: [profile.defaultLimitation], scope: { geography: 'España', period: 'Situación constitucional vigente' }, sources } }; } const profile = interpretation.criteriaProfile ? criteriaProfiles[interpretation.criteriaProfile] : undefined; const reply = `La frase «${claim}» contiene una acusación o valoración, pero por sí sola no identifica un hecho comprobable. Para sostenerla haría falta una conducta concreta, una fecha y fuentes directamente relacionadas; no es responsable convertir una coincidencia de nombres en una conclusión.`; const criteria = profile?.criteria.map((label, index) => ({ id: `criterion-${index + 1}`, label, finding: index === 0 ? 'La frase no concreta el hecho que debería comprobarse.' : 'No hay datos suficientes en la frase para evaluarlo.', sourceIds: [] })); return { state: 'insufficient', id: `interpret-${Date.now().toString(36)}`, result: { claim, interpretation, reply, answer: reply, evidenceLevel: 'insufficient', keyFact: 'La frase no especifica el hecho que debería comprobarse.', criteria, whatWeKnow: [], limitations: [profile?.defaultLimitation || 'La afirmación no tiene todavía evidencia suficiente.'], scope: {}, sources: [] } }; };
