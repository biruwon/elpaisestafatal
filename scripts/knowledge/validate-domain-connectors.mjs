...
if (imvRows.length !== 4 || !imvRows.some((row) => row.metricId === 'benefit_recipients_by_group' && row.value === 250) || !imvRows.some((row) => row.metricId === 'imv_beneficiary_average_age' && row.value === 28.4) || !imvRows.some((row) => row.dimensions?.group === 'Extranjera' && row.value === 20)) throw new Error('IMV workbook parser did not preserve beneficiaries, age, and nationality');
