...
const allowed = new Set(['covered_existing_evidence', 'covered_by_llm_evidence', 'partial_llm_evidence', 'unsupported_claim', 'covered_but_not_materialized', 'source_configured_not_refreshed', 'partial_domain_evidence', 'true_research_gap', 'unsupported_scope', 'operational_failure']);
const errors = [];
...
const expectedSourceWorkItems = report.clusters.filter((item) => !['covered_existing_evidence', 'covered_by_llm_evidence', 'partial_llm_evidence', 'unsupported_claim', 'operational_failure'].includes(item.auditClass)).length + (report.domainGaps || []).length;
...
for (const item of report.clusters || []) {
  ...
  if (['covered_existing_evidence', 'covered_by_llm_evidence', 'partial_llm_evidence'].includes(item.auditClass) && (!item.metricIds?.length && !item.linkedClaimSlug && !item.sourceIds?.length || item.action !== 'auto_route')) errors.push(`${item.clusterId}: covered item lacks evidence routing`);
