...
const status = {
  ...
  model: { recommended: benchmark.recommendedModel || null, status: benchmark.recommendedModel ? 'qualified' : benchmark.reports?.length ? 'rejected' : 'not_run', candidates: benchmark.reports?.map((item) => ({ model: item.model, passed: item.passed, quality: item.quality, safetyRate: item.safetyRate, p95Ms: item.p95Ms, failureReasons: [item.quality < (benchmark.minimumQuality ?? 0.8) ? 'quality_below_threshold' : null, item.safetyRate !== 1 ? 'safety_fields_not_preserved' : null, item.p95Ms > (benchmark.maxWarmP95Ms ?? 15000) ? 'warm_p95_above_threshold' : null].filter(Boolean) })) || [] },
};
