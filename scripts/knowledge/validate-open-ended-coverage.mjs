...
assert(demographicPension?.headline.includes('presión') && demographicPension?.headline.includes('insostenibilidad'), 'demography-pension headline did not answer both the pressure and total-unsustainability propositions');
...
const directPension = answerPlanForBroadDomain('El sistema de pensiones es completamente insostenible, y ya está arruinando las arcas públicas.');
assert(directPension?.id === 'broad-demography-pension-finance', 'direct pension-finance wording was routed to the generic pension packet');
assert(directPension?.evidenceSummary?.families.some((family) => family.label === 'Dependencia demográfica' && family.data?.some((item) => item.includes('31,2'))) && directPension?.evidenceSummary?.families.some((family) => family.label === 'Gasto total de vejez y supervivencia' && family.data?.some((item) => item.includes('205.009,77'))) && directPension?.evidenceSummary?.families.some((family) => family.label === 'Cotizaciones sociales agregadas' && family.data?.some((item) => item.includes('224.466,22'))) && directPension?.evidenceSummary?.families.some((family) => family.label === 'Saldo y deuda pública' && family.data?.some((item) => item.includes('100,7'))), 'direct pension-finance response omitted reviewed fallback data');
...
assert(demographicPensionWithData?.evidenceSummary?.families.some((family) => family.label === 'Dependencia demográfica' && family.data?.some((item) => item.includes('31,2'))), 'demography-pension packet did not render available measurements in the evidence family');
assert(!demographicPensionWithData?.blocks.some((block) => block.type === 'evidence_gap'), 'demography-pension packet exposed a pension gap despite its official account and projection coverage');
assert(demographicPensionWithData?.evidenceSummary?.families.every((family) => family.status !== 'missing'), 'demography-pension dynamic context marked a covered family as missing');
...
assert(demographicPensionWithSeries?.evidenceSummary?.families.some((family) => family.label === 'Dependencia demográfica' && family.data?.some((item) => item.includes('31,2'))) && demographicPensionWithSeries?.evidenceSummary?.families.some((family) => family.label === 'Saldo y deuda pública' && family.data?.some((item) => item.includes('100,7'))), 'demography-pension series response omitted latest compatible values');
...
assert(demographicPensionWithExpandedEvidence?.evidenceSummary?.families.some((family) => family.label === 'Proyección demográfica' && family.data?.some((item) => item.includes('Población de 65 años o más proyectada')) && family.data?.some((item) => item.includes('Población de 20 a 64 años proyectada'))), 'expanded pension evidence did not distinguish projected age groups');
assert(demographicPensionWithExpandedEvidence?.evidenceSummary?.families.some((family) => family.label === 'Gasto total de vejez y supervivencia' && family.data?.some((item) => item.includes('205.009,77'))) && demographicPensionWithExpandedEvidence?.evidenceSummary?.families.some((family) => family.label === 'Cotizaciones sociales agregadas' && family.data?.some((item) => item.includes('224.466,22'))), 'expanded pension evidence omitted total expenditure or contributions');
...
assert(demographicPensionWithExpandedEvidence?.evidenceSummary?.families.some((family) => family.label === 'Cotizaciones sociales agregadas' && !family.missingDimensions?.length), 'expanded pension evidence incorrectly treated the aggregate contributions snapshot as a pension-specific series gap');
assert(!expandedReply.includes('dato concreto sobre'), 'expanded pension response still used generic missing-data wording');
assert(demographicPensionWithExpandedEvidence?.evidenceSummary?.families.some((family) => family.label === 'Cuenta presupuestaria de la Seguridad Social' && family.data?.some((item) => item.includes('Gasto reconocido en pensiones contributivas')) && family.data?.some((item) => item.includes('Resultado presupuestario total de la Seguridad Social'))), 'expanded pension evidence omitted the official Social Security account labels');
...
assert(demographicPension?.evidenceSummary?.families.some((family) => family.label === 'Serie anual de financiación y saldo' && family.data?.some((item) => item.includes('Cotizaciones dedicadas a pensiones')) && family.data?.some((item) => item.includes('Saldo anual'))), 'pension response did not expose the annual pension-specific financing series');
assert(demographicPension?.evidenceSummary?.families.some((family) => family.label === 'Desglose presupuestario 2025P' && family.data?.some((item) => item.includes('Pensiones contributivas presupuestadas')) && family.data?.some((item) => item.includes('169.582,91'))), 'pension response did not expose the official 2025P pension budget breakdown');
assert(demographicPension?.evidenceSummary?.families.some((family) => family.label === 'Transferencias presupuestadas a pensiones' && family.data?.some((item) => item.includes('Complementos a mínimos')) && family.data?.some((item) => item.includes('19.888,00')) && !family.missingDimensions?.length), 'pension response did not expose budgeted transfer destinations without a duplicated execution gap');
assert(demographicPension?.evidenceSummary?.families.some((family) => family.label === 'Transferencias reconocidas por destino' && family.data?.some((item) => item.includes('7.928,50')) && family.data?.some((item) => item.includes('21.380,00')) && !family.missingDimensions?.length), 'pension response did not expose executed transfer destinations');
assert(demographicPension?.evidenceSummary?.families.some((family) => family.label === 'Saldos publicados del sistema' && family.data?.some((item) => item.includes('-9.310,15')) && family.data?.some((item) => item.includes('-2,48 % del PIB')) && !family.missingDimensions?.length), 'pension response did not expose complete published balances by scope');
assert(demographicPension?.evidenceSummary?.families.some((family) => family.label === 'Saldo y deuda pública' && family.data?.some((item) => item.includes('≥3 años')) && !family.missingDimensions?.length), 'pension response did not expose a measurable rule for the loaded “arruinando” claim');
assert(demographicPension?.evidenceSummary?.families.some((family) => family.label === 'Saldo y deuda pública' && family.finding?.includes('operativamente')), 'pension response did not define the operational test for “arruinando”');
