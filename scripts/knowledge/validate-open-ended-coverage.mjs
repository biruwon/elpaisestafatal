...
assert(demographicPension?.evidenceSummary?.missingDimensions?.some((item) => item.includes('saldo anual observado del sistema de pensiones')) && demographicPension?.evidenceSummary?.missingDimensions?.some((item) => item.includes('gasto contributivo neto observado')), 'demography-pension response did not preserve the remaining observed-scope gaps');
...
assert(!seriesMissing.includes('relación entre cotizantes y pensionistas') && seriesMissing.includes('saldo anual observado del sistema de pensiones') && seriesMissing.includes('gasto contributivo neto observado'), 'demography-pension series discarded genuine observed-scope gaps');
...
assert(demographicPension?.evidenceSummary?.families.some((family) => family.label === 'Transferencias presupuestadas a pensiones' && family.data?.some((item) => item.includes('Complementos a mínimos')) && family.data?.some((item) => item.includes('19.888,00')) && family.missingDimensions?.some((item) => item.includes('transferencias ejecutadas'))), 'pension response did not expose budgeted transfer destinations with their execution gap');
