...
if (missingEvidence.length || missingSources.length || missingPropositions.length || wrongClaimPropositions.length || propositionsWithoutEvidence.length || unlinkedEvidence.length) throw new Error(`Materialization requires reviewed Git records and proposition traceability. Missing evidence: ${missingEvidence.join(', ') || 'none'}; missing sources: ${missingSources.join(', ') || 'none'}; missing propositions: ${missingPropositions.join(', ') || 'none'}; propositions for another claim: ${wrongClaimPropositions.join(', ') || 'none'}; propositions without evidence: ${propositionsWithoutEvidence.join(', ') || 'none'}; proposition evidence not linked by answer: ${unlinkedEvidence.join(', ') || 'none'}.`);
