...
</style></head><body><main><header><div><p class="eyebrow">Private maintainer tool</p><h1>Knowledge gaps worth resolving.</h1><p class="muted">Generated locally from the review queue. This file is never part of the public Astro build.</p></div><p class="muted">${escapeHtml(queue?.generatedAt || 'not yet')}</p></header><div class="notice"><strong>Publication gate:</strong> popularity never publishes a claim. Review direct sources, proposition boundaries, limitations, and the build before using a promotion command.</div><section class="metrics"><div class="metric"><b>${escapeHtml(summary.candidates || 0)}</b><span>review candidates</span></div><div class="metric"><b>${escapeHtml(summary.unresolved || 0)}</b><span>unresolved</span></div><div class="metric"><b>${escapeHtml(summary.researchCandidates || 0)}</b><span>research-only gaps</span></div><div class="metric"><b>${escapeHtml(queue?.inputs?.reviewableLocalRecords || 0)}</b><span>reviewable submissions</span></div></section><div class="toolbar"><input id="filter" type="search" placeholder="Filter claims, reasons, or next actions…" aria-label="Filter knowledge gaps"><div class="tabs" role="group" aria-label="Queue type"><button type="button" data-tab="all" aria-pressed="true">All</button><button type="button" data-tab="candidate" aria-pressed="false">Review</button><button type="button" data-tab="research" aria-pressed="false">Research</button></div><span id="count" class="muted"></span></div><section id="cards" class="grid">${candidates.map((item) => card(item, 'candidate')).join('')}${research.map((item) => card(item, 'research')).join('') || (!candidates.length ? '<p class="empty">No candidates in this queue. Run <code>npm run knowledge:triage</code> first.</p>' : '')}</section></main><script>
