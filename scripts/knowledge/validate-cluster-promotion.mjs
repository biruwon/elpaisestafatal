...
const valid = validatePromotionRequest({ selector: { id: 'cluster-1' }, canonical: '¿Hay más empleo que nunca?', slug: 'empleo-record', claim });
if (valid.length) throw new Error(`Valid promotion was rejected: ${valid.join('; ')}`);

const unsafe = validatePromotionRequest({ selector: { id: 'cluster-1' }, canonical: 'No', slug: 'empleo-record', claim: { slug: 'empleo-record', status: 'planned' } });
if (unsafe.length < 2) throw new Error('Promotion did not enforce neutral wording and published-claim gates');
...
console.log('Cluster promotion validation passed: neutral wording, published-claim, and coverage gates are enforced.');
