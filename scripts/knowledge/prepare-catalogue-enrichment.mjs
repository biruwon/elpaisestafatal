...
for (const file of (await readdir(claimsDir)).filter((name) => name.endsWith('.md'))) {
  ...
  const slug = (field('slug') || file.replace(/\.md$/, '')).replace(/^['"]|['"]$/g, '');
  queue.push({ slug, claim, topics, priority: topics.some((topic) => /vivienda|empleo|salario|sanidad|corrup|inmigr/i.test(topic)) ? 'high' : 'normal' });
}
