...
const broadScorecardPattern = /\b(?:espana|pa[ií]s|este pa[ií]s|el pa[ií]s)\b[\s\w]{0,52}\b(?:destruida?|destruido|fatal|mal|ruina|desastre|cuesta abajo|arruinad[oa]|quebrada?|quiebra|bancarrota|impagable|insostenible|va peor|peor|mejorando?|progresa?|avanza?)\b|\b(?:destruy(?:e|endo)|carga)\s+espana\b|\b(?:sanchez|presidente|gobierno|moncloa|psoe|pp|vox|sumar)\b[\s\w]{0,24}\b(?:destruy|hunde?|arruin|carga|mejor|arregl|levanta|transform)\w*[\s\w]{0,12}\bespana\b|\b(?:este pa[ií]s|el pa[ií]s|espana)\s+es\s+(?:un\s+)?desastre\b|\b(?:gobernando|gobierno)\b[\s\w]{0,30}\b(?:izquierda|izquierdas|derecha)\b[\s\w]{0,30}\b(?:peor|mal|fatal)\b/i;
...
const report = { schemaVersion: '1', generatedAt: new Date().toISOString(), summary: { registryMetrics: metricAudit.length, configuredFeeds: allFeeds.length, configuredMetricCount: metricAudit.filter((item) => item.configuredFeedCount > 0).length, warehouseRecords: records.length, clusters: clusterAudit.length, newlyCoveredClusters: clusterAudit.filter((item) => item.newlyCovered).length, clusterClasses: counts(clusterAudit, 'auditClass'), metricStatuses: counts(metricAudit, 'status'), sourceWorkCandidates: sourceWork.length, sourceWorkItems: sourceWorkItems.length, refreshAttempted: Number(refreshReport.attempted || 0), refreshSucceeded: Number(refreshReport.succeeded || 0), refreshFailed: Number(refreshReport.failed || 0) }, metrics: metricAudit, clusters: clusterAudit, sourceWorkCandidates: sourceWork, sourceWorkItems, domainGaps: gapContracts.gaps || [], operationalFailures: refreshReport.failures || [], feeds: allFeeds.map(({ sourceId, metricId, url, schedule, mode, domain }) => ({ sourceId, metricId, url, schedule, mode, domain })) };
const output = pathArg('output', '.local/coverage-audit.json');
await mkdir(join(output, '..'), { recursive: true });
await writeFile(output, JSON.stringify(report, null, 2));
const markdown = [`# Coverage audit`, `Generated: ${report.generatedAt}`, '', `## Summary`, '', `- Registry metrics: ${metricAudit.length}`, `- Configured feeds: ${report.summary.configuredFeeds}`, `- Metrics with at least one configured feed: ${report.summary.configuredMetricCount}`, `- Warehouse records: ${records.length}`, `- Clusters replayed: ${clusterAudit.length}`, `- Newly covered clusters: ${report.summary.newlyCoveredClusters}`, `- Domain feeds refreshed: ${report.summary.refreshSucceeded}/${report.summary.refreshAttempted}`, `- Domain feed failures: ${report.summary.refreshFailed}`, `- Aggregate source-work candidates: ${sourceWork.length}`, `- Ranked source-work items: ${sourceWorkItems.length}`, '', '## Cluster classes', '', ...Object.entries(report.summary.clusterClasses).map(([key, value]) => `- ${key}: ${value}`), '', '## Metric readiness', '', ...Object.entries(report.summary.metricStatuses).map(([key, value]) => `- ${key}: ${value}`), '', '## Operational feed failures', '', ...(report.operationalFailures.length ? report.operationalFailures.map((item) => `- **${item.id}** — ${item.error}`) : ['- None recorded.']), '', '## Ranked source work', '', ...sourceWorkItems.slice(0, 50).map((item) => `- **${item.id}** — ${item.canonicalText}; rank ${item.rankScore} (recurrence ${item.recurrence}, harm ${item.harmScore}, urgency ${item.urgencyScore}, evidence readiness ${item.evidenceReadiness}); ${item.action}; requires ${item.requiredDimensions.join(', ')}.`), ''].join('\n');
