...
const broadScorecardPattern = /\b(?:espana|pa[ií]s|este pa[ií]s|el pa[ií]s)\b[\s\w]{0,52}\b(?:destruida?|destruido|fatal|mal|ruina|desastre|cuesta abajo|arruinad[oa]|quebrada?|quiebra|bancarrota|impagable|insostenible|va peor|peor)\b|\b(?:destruy(?:e|endo)|carga)\s+espana\b|\b(?:sanchez|presidente|gobierno|moncloa|psoe|pp|vox|sumar)\b[\s\w]{0,24}\b(?:destruy|hunde?|arruin|carga|mejor|arregl|levanta|transform)\w*[\s\w]{0,12}\bespana\b|\b(?:este pa[ií]s|el pa[ií]s|espana)\s+es\s+(?:un\s+)?desastre\b|\b(?:gobernando|gobierno)\b[\s\w]{0,30}\b(?:izquierda|izquierdas|derecha)\b[\s\w]{0,30}\b(?:peor|mal|fatal)\b/i;
