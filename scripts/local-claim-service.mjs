...
// Keep model enrichment bounded so deterministic answers are returned within
// the ordinary request budget even when Ollama is cold or under contention.
const compilerTimeoutMs = Math.min(60000, Math.max(1800, Number(process.env.LOCAL_COMPILER_TIMEOUT_MS || 9000)));
...
const stripConversationalWrapper = (value) => String(value || '')
  .replace(/^[¿?\s]*(?:es verdad que|en el grupo dicen que|mi cuñado insiste\s*:\s*|según los datos,?|no me creo que|de verdad(?: que)?|he leído esto\s*:\s*|qué hay de cierto en que)\s*/i, '')
...
const readWarehouseExcerpt = async (path, limit = 12000) => {
...
const loadWarehouse = async () => {
  ...
  warehousePromise = (async () => {
    ...
    // Include the excerpt strategy so an index generated by an older build
    // cannot keep large, unbounded source bodies resident in the resolver.
    ...
    for (const manifest of manifests) {
      try {
        ...
        // Source objects can be very large (the local warehouse is >1 GB).
        // Search needs only a bounded lead, never the complete payload.
        try { content += ` ${await readWarehouseExcerpt(manifest.objectPath)}`; } catch { /* Metadata remains searchable. */ }
...
const toResolveResult = (text, classified, source, resultRequestId = requestId(text), observations = [], eventPackets = undefined, eventEvidence = undefined) => {
  ...
  const populationContext = requestedPopulation ? observations.filter((item) => {
    const subject = normalise([item.population, JSON.stringify(item.dimensions || {}), JSON.stringify(item.dimensionLabels || {}), item.metric].filter(Boolean).join(' '));
    if (/joven|juventud/.test(requestedPopulation)) return /joven|youth|15 24|25 34|neet|tertiary/.test(subject);
    if (/mujer|hombre/.test(requestedPopulation)) return /mujer|female|male|sex/.test(subject);
    if (/mayor|pensionista/.test(requestedPopulation)) return /mayor|older|pension|65/.test(subject);
    return /famil|hogar|household/.test(subject);
  }).slice(0, 12).map((item) => ({ metricId: item.metricId, metric: item.metric, value: item.value, unit: item.unit, period: item.period, evidenceIds: [item.id].filter(Boolean), sourceId: item.source?.id })) : [];
  ...
  const claimForAnswer = boundedExcerpt(String(text || '').trim(), 220);
  const sourceLeadSummary = usableSource
    ? `No podemos confirmar la afirmación «${claimForAnswer}» con la evidencia revisada. Hemos localizado una fuente relacionada, pero no una coincidencia que mida o documente exactamente esta afirmación. La fuente aporta contexto, no una prueba directa.`
    : undefined;
  const sourceNextQuestion = usableSource
    ? (classified.guidance?.questions?.[0] || '¿Qué hecho concreto, periodo y lugar quieres comprobar?')
    : undefined;
  ...
  // A compound claim can resolve to several independent metric families. Keep
  // one compact finding per family so the answer does not silently collapse
  // into whichever family happened to rank first. The renderer can reuse the
  ...
  const compoundFamilyBlocks = compoundClaim
    ? [...observations
      ...
      .map(([metricId, items]) => {
        ...
        const first = items[0];
        const label = displayMetric(latest);
        const period = displayPeriod(latest.period || latest.id, metricId);
        const points = [
          `${label}: ${String(latest.value)} ${displayUnit(latest.unit, metricId)} (${period}).`,
        ];
  ...
  if (compilerBreakdown?.items?.length && observations.length) {
    ...
    compilerBreakdown.items = compilerBreakdown.items.map((item) => {
      ...
      const linked = observations.filter((observation) => evidenceIds.includes(observation.id));
      const evidenceText = linked.slice(0, 2).map((observation) => `${displayMetric(observation)}: ${String(observation.value)} ${displayUnit(observation.unit, observation.metricId)} (${displayPeriod(observation.period || observation.id, observation.metricId)})`).join('; ');
      const sensitiveType = ['causal', 'predictive', 'normative', 'legal'].includes(item.type);
      const finding = linked.length
        ? sensitiveType
          ? `Hay evidencia relacionada (${evidenceText}), pero no basta por sí sola para demostrar toda esta parte de la afirmación.`
          : `La evidencia localizada para esta parte es: ${evidenceText}. Debe comprobarse su definición, periodo y alcance.`
        : 'No hemos localizado una observación revisada que mida o documente exactamente esta parte de la afirmación.';
      return { ...item, evidenceIds, verdict: linked.length ? 'mixed' : 'insufficient', evidenceLevel: linked.length ? 'limited' : 'none', finding };
    });
  }
  if (compilerBreakdown?.items?.length && !observations.length) {
    compilerBreakdown.items = compilerBreakdown.items.map((item) => ({
      ...
      evidenceLevel: 'none',
      finding: 'No hemos localizado evidencia revisada suficiente para confirmar esta parte de la afirmación.',
    }));
  }
  const compoundSummary = compoundEvidenceCount > 1
    ...
    : undefined;
  const compoundAnswer = compoundClaim && compilerBreakdown?.items?.length
    ? compilerBreakdown.items.slice(0, 8).map((item) => `«${boundedExcerpt(item.text, 180)}»: ${item.finding || (item.evidenceIds?.length ? 'La evidencia relacionada requiere una comprobación específica.' : 'No hay evidencia revisada suficiente para confirmarla.')}`).join(' ')
    : undefined;
  ...
  const contextualSummary = primary ? answer : valuesContext?.summary || groupContext?.summary || quantityContext?.summary || metricContext?.summary || budgetContext?.summary || (isGovernmentEvent ? 'La comprobación debe separar el acto que se publicó de su ejecución, alcance, impacto e intención.' : undefined) || predictionContext?.summary || legalContext?.summary || definitionContext?.summary || localContext?.summary || recordedOffenceContext?.summary || causalContext?.summary || ranking?.summary || trend?.summary || (metricEvidenceGap ? 'La formulación encaja con una familia de datos reutilizable, pero el almacén local todavía no contiene observaciones compatibles para ese indicador.' : sourceLeadSummary || (relatedTopic ? `La frase parece referirse a ${relatedTopic.title.toLocaleLowerCase('es')}, pero hace falta concretar el hecho o la decisión para comprobarla.` : answer));
  const answerSummary = scorecardRequested
    ? `Comparamos seis indicadores con el último dato anterior al periodo de ${latestGovernmentPeriod.start}. No calculamos una nota global ni atribuimos causalidad al Gobierno.`
    : currentEvent
      ? 'Hemos separado el hecho, las posibles agresiones y la atribución de responsabilidad. El estado indica qué está reportado, no qué queda probado.'
      : compoundAnswer || (compoundClaim ? (compoundSummary || 'No hemos podido evaluar por separado las partes de esta afirmación.') : contextualSummary);
  const result = {
    schemaVersion: RUNTIME_VERSIONS.answerPlanSchema,
    ...
    blocks: broadConditionRequested ? [scorecardBlock, ...((requestedRegion || requestedPopulation) && scorecardBlock.items.every((item) => item.direction === 'unavailable') ? [{ type: 'scope_notice', geography: requestedRegion, population: requestedPopulation, text: 'No hay seis indicadores compatibles para este territorio o población. Mostramos solo los datos que coinciden exactamente; no usamos la media nacional como sustituto.' }] : []), ...(populationContext.length ? [{ type: 'population_context', population: requestedPopulation, observations: populationContext }] : []), ...(conditionTopics.length ? [{ type: 'condition_topics', topics: conditionTopics }] : []), ...(conditionTopicEvidence.some((topic) => topic.evidenceIds.length) ? [{ type: 'condition_topic_evidence', topics: conditionTopicEvidence }] : []), { type: 'cannot_conclude', evidenceIds: scorecardBlock.items.flatMap((item) => item.evidenceIds), points: ['El cuadro compara indicadores, no asigna una nota global a un gobierno.', 'Los cambios simultáneos no establecen qué políticas los causaron.'] }]
      ...
      : compoundClaim
      ? compactGuidanceBlocks([compilerBreakdown, ...compoundFamilyBlocks, ...(visualBlock ? [visualBlock] : []), { type: 'cannot_conclude', evidenceIds: observations.map((item) => item.id), points: ['Cada proposición se evalúa por separado; la evidencia de una parte no demuestra las demás.', 'Las analogías históricas, las predicciones y las atribuciones causales requieren pruebas específicas y no se infieren de una coincidencia temporal.'] }, { type: 'conversation_reply', evidenceIds: observations.map((item) => item.id), text: compoundAnswer || 'No hemos podido evaluar por separado las partes de esta afirmación.' }].filter(Boolean))
