...
const toResolveResult = (text, classified, source, resultRequestId = requestId(text), observations = [], eventPackets = undefined) => {
  ...
  const scorecardBlock = broadConditionRequested ? (dynamicScorecard?.items?.some((item) => item.direction !== 'unavailable') && !requestedRegion ? dynamicScorecard : snapshotScorecard('since-2018', { explicitStart: requestedYear, geography: requestedRegion ? requestedRegion : 'España' })) : null;
