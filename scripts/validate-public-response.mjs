...
const valid = publicResolveResponse({
  ...
  result: { schemaVersion: '1', headline: 'Aclaración', summary: 'Resumen', coverage: 'partial', claimType: 'mixed', evidenceLevel: 'limited', thesis: { text: 'La mejor presidencia', kind: 'evaluative', conclusion: 'Depende de los criterios', criteria: ['economía'] }, blocks: [], evidenceIds: [], sourceIds: ['source-1'], shareableReply: 'Respuesta breve', shareableSourceIds: ['source-1'], visual: { type: 'bar', title: 'Medición', unit: '%', labels: ['2025'], values: [12.5], evidenceIds: ['source-1'], sourceId: 'source-1' }, evidenceSummary: { mode: 'dynamic', families: [{ label: 'Empleo', direction: 'qualifies', evidenceIds: ['e1'] }] }, knowledgeVersion: 'test' },
});
...
if (valid.result?.shareableReply !== 'Respuesta breve' || valid.result?.shareableSourceIds?.[0] !== 'source-1') throw new Error('Shareable answer or primary-source links were not preserved at the public boundary');
if (valid.result?.visual?.sourceId !== 'source-1' || valid.result.visual.values[0] !== 12.5) throw new Error('Reviewed visual or source attribution was not preserved at the public boundary');
...
if (publicResolveResponse({ status: 'complete', result: { schemaVersion: '1', headline: 'Bad visual source', summary: 'Resumen', coverage: 'partial', claimType: 'mixed', evidenceLevel: 'limited', blocks: [], evidenceIds: [], sourceIds: ['source-1'], visual: { type: 'bar', labels: ['2025'], values: [12.5], evidenceIds: [], sourceId: 'missing-source' }, knowledgeVersion: 'test' } })) throw new Error('Unattributed visual source crossed the public boundary');
if (publicResolveResponse({ status: 'complete', result: { schemaVersion: '1', headline: 'Bad shareable source', summary: 'Resumen', coverage: 'partial', claimType: 'mixed', evidenceLevel: 'limited', blocks: [], evidenceIds: [], sourceIds: ['source-1'], shareableSourceIds: ['missing-source'], knowledgeVersion: 'test' } })) throw new Error('Unattributed shareable source crossed the public boundary');
