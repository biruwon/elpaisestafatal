...
- The scheduled public-source refresh now optionally loads the derived warehouse into PostgreSQL and runs the complete 341-case observation-level benchmark when `WAREHOUSE_DATABASE_URL` is configured. It validates recall, negative-query rejection, weak-evidence safety, errors, and latency, then stores a short-lived private artifact; credentialless refreshes continue to run the JSON warehouse path only.
...
Current high-risk domain feeds: the INE population denominator feed is active; Interior's annual recorded-offences CSV is active; and Social Security's IMV dossier is active for title holders by Spanish/foreign nationality. These feeds are versioned into the local warehouse with typed period, geography, group, category, and metric fields. Public-housing allocation remains a discovery-only gap because the national programme portal exposes reporting templates and regional submissions rather than a consolidated allocation-by-group dataset.
