...
- Twenty-five starter claim families now cover everyday immigration, housing, employment, healthcare, prices, taxes, inequality, security, politics, and economic debates. They reuse the existing reviewed evidence warehouse and are exposed through the compact popular-entry set and topic filters.
...
- `bge-m3`, `gemma3:4b`, and `qwen3-vl:8b` are installed on the development machine. The current local evaluation corpus contains 420 Spanish inputs across 11 categories; the latest run reached 297/300 known-family retrieval and 116/120 unknown-safety cases, with the remaining misses retained as follow-up long-tail regressions rather than hidden.
...
- The shipped refresh now has a validated default configuration covering thirteen live resources: BOE publications, INE CPI, and Eurostat GDP, Spain and cross-country unemployment, employment, population, migration population, immigration flows, recorded offences, regional density, harmonised prices, and quarterly house prices; refresh runs locally first from `.local/source-refresh.json` and falls back to the versioned configuration.
...
- The homepage prompt starter now covers additional published everyday claims across healthcare access, unemployment definitions, inflation, housing prices, irregular arrivals, tax aggregates, housing ownership, construction, employment quality, cybercrime, poverty, and political concern; compact topic filters keep those prompts discoverable without turning the checker into a wall of links.
...
- The conversation starter library now covers 25 published, evidence-backed claim families across immigration, housing, employment, healthcare, prices, taxes, inequality, security, politics, and everyday economic debates. The homepage keeps the first eight prompts scannable, then lets people reveal the rest or filter by topic instead of creating a wall of prompts.
...
- The built claim index now has a CI/build regression gate covering representative aliases for the 25 starter families, including unique canonical ownership and variant presence; missing or ambiguous routing coverage fails the build before publication.
