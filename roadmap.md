...
- 257 Markdown claim records exist; 79 are published.
- The 79 published claims now expose typed, evidence-linked core propositions; the browser index carries proposition IDs, the local resolver carries them into published answer plans, and the build validates the claim → proposition → evidence → source chain.
- All 79 published claim families now have structured conversation definitions covering everyday immigration, housing, employment, healthcare, tourism, prices, taxes, inequality, security, politics, youth, corruption, legal rules, demographic change, education, and economic debates. They reuse the existing reviewed evidence warehouse and are exposed through the compact popular-entry set and topic filters.
...
- A new static `/datos` catalogue exposes all 40 configured metric/document families with searchable subject filters, human-readable units/populations, complete one-click checker prompts, and a clear distinction between evidence families and verdicts, making warehouse coverage discoverable without adding another long homepage prompt list.
...
- `bge-m3`, `gemma3:4b`, and `qwen3-vl:8b` are installed on the development machine. The current local evaluation corpus contains 876 Spanish inputs across 13 categories; the last completed runtime run reached 744/744 known-family retrieval, 60/60 unknown-safety cases, 0 irrelevant matches, and 804/804 traceable outcomes before the latest education, healthcare-access, GDP-per-capita, and absolute-debt cases. The education, youth, healthcare-access, GDP-per-capita, and absolute-debt families now have runtime smoke coverage, while adjacent-letter transposition typos resolve to the intended published family, local/private claims suppress unrelated national guidance and sources, and published results carry reviewed HTTPS source links into the answer plan.
...
- Published claim pages now use the same deterministic accessible SVG/value-table chart pattern as dynamic answers, replacing text-only evidence bars across all 79 published claim pages.
...
- The built claim index now has a CI/build regression gate covering representative aliases for all 79 published families, including unique canonical ownership and variant presence; missing or ambiguous routing coverage fails the build before publication.
...
The homepage now exposes twenty-two warehouse-backed discovery cards across cost of living, macroeconomics, housing, employment, health, security, territory, public debt, youth unemployment, demography, inequality, public revenue, public expenditure, household income, ageing, early school leaving, tertiary education, NEET, and unmet healthcare needs. GDP per capita and absolute public debt are available through the progressive long-tail prompt set and `/datos` catalogue while the six high-value starters remain visible, so current-data coverage stays discoverable without turning the homepage into a wall of cards. These are reusable data entry points rather than manually authored verdicts. The `/datos` catalogue provides the longer-tail discovery layer for all configured metric families without turning the main checker into a catalogue.
