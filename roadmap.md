...
- A new static `/datos` catalogue exposes all 53 configured metric families with searchable subject filters, human-readable units/populations, complete one-click checker prompts, and a clear distinction between evidence families and verdicts, making warehouse coverage discoverable without adding another long homepage prompt list. This now includes the statutory minimum-wage series as a distinct measure from household income and employment, plus social-protection benefits expenditure, Spain-only old-age/survivors benefits expenditure, the Spain/EU old-age and survivors comparison, the Spain/EU current-taxes-on-income-and-wealth comparison, and residents by foreign citizenship as distinct aggregates from total government spending, total public revenue, household income, individual tax bills, country of birth, and annual arrivals.
...
- A reproducible 300-case Spanish warehouse-routing benchmark now covers fifty-two statistical metric families plus ten out-of-domain inputs. With the installed local embedding baseline, the corpus validator verifies all 300 Spanish paraphrases; batched embedding requests keep larger evaluation sets within the local runtime limits, explicit metric hints win when semantic neighbours disagree, hybrid recall@3 reaches 280/290 positive cases, and the full runtime smoke checks rejection of out-of-domain inputs and unsafe unrelated matches. The regression set includes citizenship wording that must not fall through to country-of-birth or annual-arrivals data, as well as minimum-wage wording that must not fall through to household income, pension wording that must not fall through to total social-protection spending, Spain/EU pension-comparison wording that must not fall through to Spain-only pension spending, current-tax wording that must not fall through to total public revenue or a household tax bill, and social-protection wording that must not fall through to total government spending.
...
The homepage now exposes thirty-seven compact warehouse-backed discovery cards across cost of living, macroeconomics, housing, employment, statutory minimum wage, social protection, pensions, health, security, territory, public debt, youth unemployment, demography, inequality, public revenue, public expenditure, household income, ageing, early school leaving, tertiary education, NEET, unmet healthcare needs, foreign citizenship, and Spain-versus-EU growth, inflation, employment, revenue, expenditure, health spending, median income, GDP per capita, old-age/survivors spending, and current taxes on income and wealth. The checker also provides sixty-three progressive long-tail prompts and the `/datos` catalogue exposes every configured metric family, while the six high-value starters remain visible so current-data coverage stays discoverable without turning the homepage into a wall of cards. These are reusable data entry points rather than manually authored verdicts.
