...
- 222 Markdown claim records exist; 40 are published.
- The 40 published claims now expose one typed, evidence-linked core proposition each; the browser index carries proposition IDs, the local resolver carries them into published answer plans, and the build validates the claim → proposition → evidence → source chain.
- All 40 published claim families now have structured conversation definitions covering everyday immigration, housing, employment, healthcare, prices, taxes, inequality, security, politics, youth, corruption, and economic debates. They reuse the existing reviewed evidence warehouse and are exposed through the compact popular-entry set and topic filters.
...
- 38 evidence records and 31 source records are currently linked.
...
- `bge-m3`, `gemma3:4b`, and `qwen3-vl:8b` are installed on the development machine. The current local evaluation corpus contains 528 Spanish inputs across 11 categories; the latest run reaches 408/408 known-family retrieval, 120/120 unknown-safety cases, 0 irrelevant matches, and 408/528 traceable outcomes. Adjacent-letter transposition typos now resolve to the intended published family, while local/private claims suppress unrelated national guidance and sources. The remaining traceability gap belongs to uncovered/no-evidence responses and remains an intentional follow-up for richer structured decomposition.
...
- Published claim pages now use the same deterministic accessible SVG/value-table chart pattern as dynamic answers, replacing text-only evidence bars across all 40 published claim pages.
...
- The built claim index now has a CI/build regression gate covering representative aliases for all 40 published families, including unique canonical ownership and variant presence; missing or ambiguous routing coverage fails the build before publication.
