...
- A new static `/datos` catalogue exposes all 34 configured metric/document families with searchable subject filters, human-readable units/populations, complete one-click checker prompts, and a clear distinction between evidence families and verdicts, making warehouse coverage discoverable without adding another long homepage prompt list.
...
- Published claim pages now use the same deterministic accessible SVG/value-table chart pattern as dynamic answers, replacing text-only evidence bars across all 73 published claim pages.
...
- The conversation starter library now covers all 73 published, evidence-backed claim families across immigration, housing, employment, tourism, prices, taxes, inequality, security, politics, youth, corruption, legal rules, demographic change, and everyday economic debates. The homepage keeps the first eight prompts scannable, then lets people reveal the rest or filter by topic instead of creating a wall of prompts.
...
- The built claim index now has a CI/build regression gate covering representative aliases for all 73 published families, including unique canonical ownership and variant presence; missing or ambiguous routing coverage fails the build before publication.
...
The homepage now exposes eighteen warehouse-backed discovery cards across cost of living, macroeconomics, housing, employment, health, security, territory, public debt, youth unemployment, demography, inequality, public revenue, public expenditure, household income, and ageing. These are reusable data entry points rather than manually authored verdicts. The `/datos` catalogue provides the longer-tail discovery layer for all configured metric families without turning the main checker into a catalogue.
