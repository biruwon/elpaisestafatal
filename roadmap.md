...
- A new static `/datos` catalogue exposes all 48 configured metric/document families with searchable subject filters, human-readable units/populations, complete one-click checker prompts, and a clear distinction between evidence families and verdicts, making warehouse coverage discoverable without adding another long homepage prompt list. This now includes the statutory minimum-wage series as a distinct measure from household income and employment.
...
- A reproducible 117-case Spanish warehouse-routing benchmark now covers forty-seven statistical metric families plus ten out-of-domain inputs. With the installed `bge-m3` baseline, the corpus validator verifies all 117 Spanish paraphrases and preserves explicit metric hints when semantic neighbours disagree; the full runtime smoke also checks rejection of out-of-domain inputs and unsafe unrelated matches, including minimum-wage wording that must not fall through to household income.
...
The homepage now exposes thirty-two compact warehouse-backed discovery cards across cost of living, macroeconomics, housing, employment, statutory minimum wage, health, security, territory, public debt, youth unemployment, demography, inequality, public revenue, public expenditure, household income, ageing, early school leaving, tertiary education, NEET, unmet healthcare needs, and Spain-versus-EU growth, inflation, employment, revenue, expenditure, health spending, median income, and GDP per capita. The checker also provides fifty-eight progressive long-tail prompts and the `/datos` catalogue exposes every configured metric/document family, while the six high-value starters remain visible so current-data coverage stays discoverable without turning the homepage into a wall of cards. These are reusable data entry points rather than manually authored verdicts.
