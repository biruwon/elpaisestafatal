...
## Model-generated claim promotion

Every model-generated catalogue entry is enriched during the same generation run whenever evidence research is possible; it does not wait for repeated demand. Codex formulates neutral research queries, retrieves primary sources, stores the publisher, URL, retrieval date, exact excerpt or dataset cells, and links observations to the decisive sentences. The existing entry is upgraded in place only when deterministic gates confirm citation coverage, exact numbers, compatible units/period/geography/population, visual consistency, source snapshots, and no overclaiming. Entries that cannot be sourced in that run remain searchable but visibly labelled `Respuesta generada por IA`; model generation alone never changes provenance. Repeated demand may improve prioritization, but is not a prerequisite for enrichment.
...
- Semantic routing now requires a specific relation, multiple concepts, or multiple fallback terms before it can promote a paraphrase to a published claim. Broad inputs such as `La economía y el empleo van a peor` remain uncovered with related context instead of being mislabelled as a specific unemployment claim; the same guard is enforced in the local resolver.
- Deployment verification now waits on the lightweight canonical health boundary during edge propagation and runs the full production smoke suite exactly once afterward, avoiding self-inflicted classifier rate limits during retries.
- The local semantic index cache is now invalidated by the central index and deterministic-compiler versions as well as catalogue/model changes, so parser and safety-rule updates cannot reuse stale routing artifacts.
- The deterministic Spanish compiler now recognizes plural worsening/improving forms such as `van a peor` and `van a mejor` consistently in claim type, trend direction, and semantic-family signatures.
- Everyday causal connectors such as `porque`, `ya que`, `debido a que`, and `por culpa de` now split novel compound claims into separate explicit premises while preserving a causal family signature in both the local compiler and browser semantic index.
...
- The server-side `/api/check` boundary now shares the published claim index for strong text matches. Exact published claims resolve immediately without contacting optional inference, keeping browser and server resolution consistent while leaving uncovered and media inputs on the existing fallback path.
...
- Deployed the validated Pages build to production and reran the live smoke contract. The public `/api/check` boundary now returns topic-only political context for uncovered political complaints, with no fabricated evidence or unrelated claim, and the live site matches the pushed source state.
...
- Removed the obsolete duplicate clarifier route and its hand-maintained MVP catalogue. Published claims now have one canonical source (`content/claims` → `claimCatalog` → `publishedCatalogueEntries`) and one public answer route (`/afirmaciones/[slug]`).
...
- The Pages `/api/check` boundary now returns an evidence-honest deterministic clarification when the optional inference origin is absent or fails, including a concrete follow-up question for typed claims and a generic retry path for file-only submissions; direct API callers no longer receive a dead-end merely because dynamic inference is unavailable.
...
- `/api/check` is the single public provider-neutral classifier boundary used by the claim input, with JSON/multipart validation, timeout, token forwarding, generic failures, and bounded polling.
...
- A reproducible backup command now exports operational D1 state, the local source warehouse, and configuration/migration manifests into ignored `.local/backups` artifacts.
...
- Expanded the local compiler's reviewed candidate context with claim type, geography, period, and bounded aliases instead of title-only routing hints. The local model can now distinguish a genuine paraphrase such as `Desde que llegaron más extranjeros hay más inseguridad` from a merely related topic; the deterministic handler, score, numeric, compatibility, evidence, and publication gates still decide the returned result. Added a local smoke regression for this model-enriched route. Further model-enriched coverage remains pending for compound and genuinely novel claim families.
...
```http
POST /api/check
GET  /api/check/:requestId
```

The frontend submits once and polls automatically when a request is processing. There is no second classification click.

The classifier has one public `/api/check` route and one local `/v1/classify` contract; no compatibility classifier routes are retained.
...
Status: in progress. Quantity, proportion, trend, ranking, definition, budget-transfer, government-event, causal, legal, group-comparison, prediction, and normative routing now exist. Long-tail causal, legal, group-comparison, predictive, normative, and official-event inputs produce dedicated deterministic method plans instead of a generic unsupported answer. The legal handler now prefers the explicitly current BOE article version and cites its exact text while retaining scenario, exception, and jurisprudence limitations; deeper domain-specific calculations and broader legal resolution remain pending.
...
Status: in progress. Unknown measurable claims progressively use the local warehouse and constrained official discovery. Unknown legal claims now search BOE's consolidated-legislation collection with bounded wildcard title pairs, preserve user-specific terms alongside formal expansions, reject expired or outdated consolidations, download at most two bounded official texts, select only current directly matching articles, cache the result, and retain scenario/jurisprudence limitations. Public-information reuse variants are recognized as one reusable legal family, with operative-article ranking and a source-backed conversational response instead of a generic legal disclaimer. Budget-transfer variants such as “Bolaños se lleva 310 millones de Educación” and “Educación pierde 310 millones para Presidencia” now share the official-event parser and money-flow answer, with unsupported service-cut and adviser implications kept explicit. Newly published official decisions, appointments, grants, and similar acts now receive a bounded provisional event response instead of generic source context, while the publication, execution, impact, and intention layers remain separate. Evaluation-driven routing now resolves typo variants without combining unrelated aliases, and refuses to attach national context to local/private claims that require unavailable records. Persistent knowledge-gap promotion is implemented; remaining long-tail work is source-specific coverage, not automatic publication.
...
The solo-maintainer handoff is now reproducible with `npm run knowledge:triage`: it combines local gap records with an optional explicit `--export-d1` production snapshot, clusters and reconciles them against the published catalogue, then writes one ranked JSON/Markdown review queue. The scheduled workflow applies `npm run d1:migrate` first, then uses the separate explicit `--sync-d1` flag to persist maintainer-only triage status to the originating D1 clusters; production export and sync are opt-in and the command never publishes or promotes a claim.
...
- Run production smoke checks against both static routes and the generic `/api/health` and `/api/check` boundaries without requiring dynamic inference to be available.
