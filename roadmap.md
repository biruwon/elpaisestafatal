...
- The homepage now exposes a compact “datos actualizados” prompt rail for inflation, inequality, public deficit, household income, and demographic change, so visitors can discover dynamic evidence-backed questions without mistaking them for published claim pages.
...
- A reproducible 82-case Spanish warehouse-routing benchmark now covers thirty-one statistical metric families plus ten out-of-domain inputs. With the installed `bge-m3` baseline, hybrid retrieval reaches 72/72 top-1 metric routing versus 55/72 lexical, retains 72/72 recall@3, rejects 10/10 out-of-domain inputs, and produces no unsafe top match; explicit metric hints are preserved when semantic neighbours disagree.
