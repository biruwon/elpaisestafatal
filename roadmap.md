...
The homepage now exposes thirty-nine compact warehouse-backed discovery cards across cost of living, macroeconomics, housing, employment, statutory minimum wage, social protection, pensions, health, security, territory, public debt, youth unemployment, demography, inequality, public revenue, public expenditure, household income, ageing, early school leaving, tertiary education, NEET, unmet healthcare needs, foreign citizenship, part-time employment, and Spain-versus-EU growth, inflation, employment, revenue, expenditure, health spending, median income, GDP per capita, old-age/survivors spending, current taxes on income and wealth, and part-time employment. The checker also provides sixty-five progressive long-tail prompts and the `/datos` catalogue exposes every configured metric family, while the six high-value starters remain visible so current-data coverage stays discoverable without turning the homepage into a wall of cards. These are reusable data entry points rather than manually authored verdicts.
...
The current-data layer now includes part-time employment in Spain and the Spain–EU comparison. Its reviewed starter claim distinguishes the share of existing employment from the stronger, unsupported statement that all newly created jobs are part-time.
