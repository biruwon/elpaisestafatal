...
- 253 Markdown claim records exist; 75 are published.
- The 75 published claims now expose typed, evidence-linked core propositions; the browser index carries proposition IDs, the local resolver carries them into published answer plans, and the build validates the claim → proposition → evidence → source chain.
- All 75 published claim families now have structured conversation definitions covering everyday immigration, housing, employment, healthcare, tourism, prices, taxes, inequality, security, politics, youth, corruption, legal rules, demographic change, education, and economic debates. They reuse the existing reviewed evidence warehouse and are exposed through the compact popular-entry set and topic filters.
...
- 71 evidence records and 64 source records are currently linked; the new education claims add two official Eurostat series with explicit population definitions, periods, limitations, and proposition links.
...
- A new static `/datos` catalogue exposes all 36 configured metric/document families with searchable subject filters, human-readable units/populations, complete one-click checker prompts, and a clear distinction between evidence families and verdicts, making warehouse coverage discoverable without adding another long homepage prompt list.
...
- `bge-m3`, `gemma3:4b`, and `qwen3-vl:8b` are installed on the development machine. The current local evaluation corpus contains 828 Spanish inputs across 13 categories; the last completed runtime run reached 744/744 known-family retrieval, 60/60 unknown-safety cases, 0 irrelevant matches, and 804/804 traceable outcomes before the latest education cases. The new education families now have runtime smoke coverage, while adjacent-letter transposition typos resolve to the intended published family, local/private claims suppress unrelated national guidance and sources, and published results carry reviewed HTTPS source links into the answer plan.
...
- The shipped refresh now has a validated default configuration covering thirty-five live resources: BOE publications, INE CPI, and Eurostat macroeconomic, labour, population, migration, poverty, crime, housing, rental-price, health, life-expectancy, fertility, ageing, older- and young-population shares, population change, inflation, inequality, income, electricity-price, public-finance, early-school-leaving, and tertiary-attainment series; refresh runs locally first from `.local/source-refresh.json` and falls back to the versioned configuration.
...
- Published claim pages now use the same deterministic accessible SVG/value-table chart pattern as dynamic answers, replacing text-only evidence bars across all 75 published claim pages.
...
- The built claim index now has a CI/build regression gate covering representative aliases for all 75 published families, including unique canonical ownership and variant presence; missing or ambiguous routing coverage fails the build before publication.
...
The homepage now exposes twenty warehouse-backed discovery cards across cost of living, macroeconomics, housing, employment, health, security, territory, public debt, youth unemployment, demography, inequality, public revenue, public expenditure, household income, ageing, early school leaving, and tertiary education. These are reusable data entry points rather than manually authored verdicts. The `/datos` catalogue provides the longer-tail discovery layer for all configured metric families without turning the main checker into a catalogue.
