...
The comparison pages also exercise the reusable labelled comparison visual with units, period, evidence limit, and accessible value-table fallback. Claim evidence trails now expose each linked record's concise finding inline, plus its own limitation where the record provides one, so users do not need to open a source merely to understand what it establishes. Broader reviewed chart coverage and reusable animated scenes remain future work.
