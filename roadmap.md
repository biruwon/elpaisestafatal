...
This iteration adds four reviewed Spain-versus-EU comparison families—old-age/survivors benefits, current taxes on income and wealth, total social-protection benefits per inhabitant, and public deficit—as static claim pages backed by the same warehouse metrics. It also adds the reviewed youth-unemployment comparison family, extending the existing youth level and trend coverage into a same-period international comparison. They demonstrate the intended scale path: promote high-demand reusable evidence into concise pages without creating a bespoke investigation for every phrasing.
