...
- The shipped refresh now has a validated default configuration covering fifteen live resources: BOE publications, INE CPI, and Eurostat current-price GDP, real GDP growth, Spain and cross-country unemployment, employment, population, migration population, AROPE, immigration flows, recorded offences, regional density, harmonised prices, and quarterly house prices; refresh runs locally first from `.local/source-refresh.json` and falls back to the versioned configuration.
...
- A reproducible 54-case Spanish warehouse-routing benchmark now covers sixteen statistical metric families plus ten out-of-domain inputs. With the installed `bge-m3` baseline, hybrid retrieval reaches 43/44 top-1 metric routing versus 27/44 lexical, retains 44/44 recall@3, rejects 10/10 out-of-domain inputs, and produces no unsafe top match.
...
- Eurostat coverage now includes real GDP growth, AROPE, public-debt-to-GDP, and youth-unemployment families with localized display titles, source refresh definitions, retrieval benchmarks, and resolver smoke coverage.
...
Status: in progress. The shared renderer now supports claim breakdowns, key numbers, accessible inline SVG trend/comparison charts with expandable value tables, money flows, confirmed/unknown cards, source excerpts, strongest-valid-concern cards, evidence ladders, legal decision paths, prediction conditions, normative trade-offs, group-comparability checklists, conversation replies, source links, and answer-first topic-page chapters. Both dynamic answers and all published claim pages use the same deterministic chart/value-table pattern. Broader reviewed chart coverage and reusable animated scenes remain pending.
