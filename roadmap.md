...
- 255 Markdown claim records exist; 77 are published.
- The 77 published claims now expose typed, evidence-linked core propositions; the browser index carries proposition IDs, the local resolver carries them into published answer plans, and the build validates the claim → proposition → evidence → source chain.
- All 77 published claim families now have structured conversation definitions covering everyday immigration, housing, employment, healthcare, tourism, prices, taxes, inequality, security, politics, youth, corruption, legal rules, demographic change, education, and economic debates. They reuse the existing reviewed evidence warehouse and are exposed through the compact popular-entry set and topic filters.
...
- 73 evidence records and 66 source records are currently linked; the education and youth claims add three official Eurostat series, and the healthcare-access slice adds a fourth, all with explicit population definitions, periods, limitations, and proposition links.
...
- A new static `/datos` catalogue exposes all 38 configured metric/document families with searchable subject filters, human-readable units/populations, complete one-click checker prompts, and a clear distinction between evidence families and verdicts, making warehouse coverage discoverable without adding another long homepage prompt list.
...
- `bge-m3`, `gemma3:4b`, and `qwen3-vl:8b` are installed on the development machine. The current local evaluation corpus contains 852 Spanish inputs across 13 categories; the last completed runtime run reached 744/744 known-family retrieval, 60/60 unknown-safety cases, 0 irrelevant matches, and 804/804 traceable outcomes before the latest education and healthcare-access cases. The education, youth, and healthcare-access families now have runtime smoke coverage, while adjacent-letter transposition typos resolve to the intended published family, local/private claims suppress unrelated national guidance and sources, and published results carry reviewed HTTPS source links into the answer plan.
...
- Published claim pages now use the same deterministic accessible SVG/value-table chart pattern as dynamic answers, replacing text-only evidence bars across all 77 published claim pages.
...
- The built claim index now has a CI/build regression gate covering representative aliases for all 77 published families, including unique canonical ownership and variant presence; missing or ambiguous routing coverage fails the build before publication.
...
The homepage now exposes twenty-two warehouse-backed discovery cards across cost of living, macroeconomics, housing, employment, health, security, territory, public debt, youth unemployment, demography, inequality, public revenue, public expenditure, household income, ageing, early school leaving, tertiary education, NEET, and unmet healthcare needs. Six high-value starters remain visible and the remaining cards sit behind a native accessible disclosure, so current-data coverage stays discoverable without turning the homepage into a wall of cards. These are reusable data entry points rather than manually authored verdicts. The `/datos` catalogue provides the longer-tail discovery layer for all configured metric families without turning the main checker into a catalogue.
...
The warehouse now also exposes the nominal GDP series through everyday wording such as `tamaño de la economía española`, keeps it separate from real GDP growth, localizes the unit as `millones de euros`, and surfaces it in the homepage data prompts.
