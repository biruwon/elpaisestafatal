...
- The homepage now exposes a compact “datos actualizados” prompt rail for electricity prices, inflation, inequality, public deficit, household income, and demographic change, so visitors can discover dynamic evidence-backed questions without mistaking them for published claim pages.
...
- A reproducible 84-case Spanish warehouse-routing benchmark now covers thirty-two statistical metric families plus ten out-of-domain inputs. With the installed `bge-m3` baseline, hybrid retrieval reaches 74/74 top-1 metric routing versus 56/74 lexical, retains 74/74 recall@3, rejects 10/10 out-of-domain inputs, and produces no unsafe top match; explicit metric hints are preserved when semantic neighbours disagree.
