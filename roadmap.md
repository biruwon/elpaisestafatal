...
- The shipped refresh now has a validated default configuration covering twenty-seven live resources: BOE publications, INE CPI, and Eurostat macroeconomic, labour, population, migration, poverty, crime, housing, health, life-expectancy, fertility, ageing, inequality, income, and public-finance series; refresh runs locally first from `.local/source-refresh.json` and falls back to the versioned configuration.
...
- The homepage now exposes a compact “datos actualizados” prompt rail for inequality, public deficit, and household income, so visitors can discover dynamic evidence-backed questions without mistaking them for published claim pages.
...
- A reproducible 74-case Spanish warehouse-routing benchmark now covers twenty-six statistical metric families plus ten out-of-domain inputs. With the installed `bge-m3` baseline, hybrid retrieval reaches 62/64 top-1 metric routing versus 46/64 lexical, retains 64/64 recall@3, rejects 10/10 out-of-domain inputs, and produces no unsafe top match.
...
- Eurostat coverage now includes the old-age dependency ratio, with a dedicated ageing metric, source refresh definition, Spanish routing aliases, localized unit (`personas mayores por cada 100 en edad de trabajar`), benchmark cases, and resolver smoke coverage. It is kept distinct from total population, fertility, and life expectancy.
...
- Published claim pages now use the same deterministic accessible SVG/value-table chart pattern as dynamic answers, replacing text-only evidence bars across all 37 published claim pages.
...
- The built claim index now has a CI/build regression gate covering representative aliases for all 37 published families, including unique canonical ownership and variant presence; missing or ambiguous routing coverage fails the build before publication.
...
- Structured result cards now make the short answer and copy action prominent, while secondary method, source-excerpt, legal, prediction, trade-off, and comparability blocks are grouped inside an accessible expandable analysis section. This keeps the first answer useful without hiding the evidence path.
