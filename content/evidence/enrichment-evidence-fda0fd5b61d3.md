---
id: enrichment-evidence-fda0fd5b61d3
kind: verified-source-excerpt
sourceIds: ["enrichment-source-e6d539b7a74e"]
period: "unspecified"
geography: "unspecified"
unit: "unspecified"
---

The retrieved source excerpt is:

> Vinculadas al sector público empresarial y fundacional, será la Intervención General de la Administración del Estado (IGAE) la que las controle a través de informes para su nombramiento y cese, además de ocuparse del establecimiento de contenidos mínimos comunes y la supervisión del sistema. Bajo la tutela de la IGAE, existirá un sistema de participación para controlar la labor del sector público estatal, para el qu…

The excerpt has not been validated as sufficient to establish the full claim.
